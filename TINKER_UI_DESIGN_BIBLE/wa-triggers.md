...
- **Profile authorship is agent-driven, not user-driven.** Don't add a manual-seed step that requires the user to characterise their hundreds of chats. The lazy/observational pattern is intentional — see the user's 2026-05-09 note in `~/.openclaw/workspace/memory/knowledge/whatsapp-strategy.md`. Cron-based bulk drafting was rejected (stakes too varied, drafts too risky). Reverting to a manual-seed model would defeat the design.
