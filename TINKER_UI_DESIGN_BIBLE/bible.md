...
- **Status:** `DESIGN — building` (owner-approved 2026-06-11; supersedes the in-band injection of §5.67 — flip §5.67 to `SUPERSEDED` when v2 ships). Build the whole thing in one drop, then refine by use (owner decision). Full design history + file-level grounding: jarvis-icu commit `b9aedcf` + three code-verified research passes.
