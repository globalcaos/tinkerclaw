...
The papers themselves are the **WANT** (design-principles #23). This table is **RUN** — an observation of how much of each paper is currently firing, dated, never a reason to drop the paper. When RUN says dead-on-arrival, the paper is the drawing board for the next attempt, not a retired idea. `DEPLOYED` in a cell below means "something with that name is running," not "the paper is satisfied." The 2026-08-02 J-series registry (`jarvis-icu/docs/notes/2026-08-02-jseries-implementation-registry.md`) refuted 74 of 155 claimed-live mechanisms; several rows here still say `DEPLOYED` against that audit (Humor/LIMBIC, Fractal-as-AMYGDALA, Round Table, Identity, Sleep). Flip RUN when evidence arrives; do not delete the row.

11 papers in `docs/papers/` (the series now runs J1–J20; this table is the original eleven and is a RUN snapshot, not the gift list). Status of each relative to deployed code:
