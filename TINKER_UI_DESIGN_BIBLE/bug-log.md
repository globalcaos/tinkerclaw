...
### FIXED [spec-drift+display-misclassify]: the-voice-script-authored-the-purple-line-a-second-time (2026-09-06)

- **Symptom:** the owner, in the torrent-scout tab: _"In the torrent skill tab I am using right now, I get more than one answer most of the time. This is a problem we've been trying to fix multiple times. What I recall from those times is that we have not detected double tokens being used, so we presume it is basically a front-end bug, but we've been unable to locate its origin."_
- **The measurement.** Session `agent:main:tinker:mtocsumz` (`54d1b065-….jsonl`): **14 user messages, 21 assistant messages.** Seven user turns carry TWO assistant siblings under one `parentId` — one `provider:"claude-code"` / `model:"claude-opus-5"` with thousands of output tokens, and one `provider:"openclaw"` / `model:"gateway-injected"` with `usage.output: 0`. The injected texts are in the spoken register ("thirty-two consecutive", "forty-four times", "two thousand seventeen") because they are the string handed to TTS. They fired on exactly the seven turns where the model ran `jarvis "…"` as a Bash call — one-to-one, with a consistent 5.3–5.8 s lag from exec start to inject timestamp. Five of the seven duplicated a `**Jarvis:**` line the model had already written in its own reply; not one was the rescue the block existed to provide.
- **Why "most of the time" and not "every time" — a second, silent bug on the same line.** The inject was built as `setsid bash -c "openclaw gateway call chat.inject --params '$PARAMS' …"`, nesting a single-quoted JSON blob inside a double-quoted `-c` string. Any apostrophe in the spoken text broke the quoting and the command died without a trace: the 2026-09-05 13:46:59 voice call, whose text contained "Sasha's", produced no bubble at all. So roughly thirteen turns called `jarvis` and only seven surfaced a duplicate — the rate was set by how often the persona happened to use a contraction. The same line also raced the transcript's creation once (`createIfMissing`), writing a duplicate session header and a `parentId: null` orphan bubble. Both are fixed alongside the gate: the call no longer goes through `bash -c`, so the opt-in path actually works.
- **Root cause — the identity line had two authors, and only one of them was still supposed to exist.** `~/.local/bin/jarvis` (the TTS script, untracked, in neither repo) ran `openclaw gateway call chat.inject` whenever `$TC_SESSION_KEY` was set — which `tinkerclaw-tinker-bridge/src/worker.ts:847` sets for every Tinker turn, and whose channel gate (`VOICE_CHANNELS="main tinker"`) explicitly passes `tinker`. The gateway handler (`src/gateway/server-methods/chat.ts:3105-3167`) then does both halves of the damage: it persists a real `role:"assistant"` record via `chat-transcript-inject.ts`, and it broadcasts that record to the UI as `state:"final"`. Meanwhile doctrine had been reversed on 2026-08-31: `workspace/IDENTITY.md` (`speak_on_home_session`) and `jarvis-voice/VOICE.md` both say **"Purple is in the reply. Audio is extra."** and, verbatim, "Do **not** wait for the script to `chat.inject` the bubble." The script was never updated to match, so both authors ran unconditionally and the line was written twice. The script's own comment recorded the obsolete trade it was still honouring: _"Duplicate bubbles are cheaper than a missing identity line in the reply."_
- **Why every previous hunt failed — three independent decoys.** (a) `usage` is hard-coded to all-zero in `chat-transcript-inject.ts`, so no second model call was ever billed and every "are we paying twice?" check came back correctly clean, which is what fixed the front-end hypothesis in place. (b) The producer is a **bash script outside both git repos**, so no amount of grepping `tinkerclaw` or `.openclaw` could reach it — "unable to locate its origin" was literally true. (c) A 2026-07-31 fix DID teach the UI about these sentinels, and it looked like the relevant fix: `tinker-ui/src/transcript-only-models.ts` exports `isTranscriptOnlyModel`, and the file's own header is about `gateway-injected`. But it is consulted at exactly two call sites, both inside `renderThinkingIndicator()`, where it only picks the spinner's model name and colour. It has never touched bubble rendering. A second decoy fix, FORK 2026-08-05 ("duprep — the 'I see my answer twice' report", `src/gateway/server-chat.ts`), was real but addressed a different mechanism entirely: client cursors indexing into a server-cumulative delta buffer.
- **Fix:** the inject is now behind `TC_VOICE_BUBBLE`, default off (`if [ "${TC_VOICE_BUBBLE:-0}" = "1" ] && [ -n "${TC_SESSION_KEY:-}" ]`). Gated rather than deleted so the rescue case — a turn where the model omits the inline line — stays one env var away. The audio path is untouched. No rebuild and no gateway restart were required: the producer is a shell script, and the UI at :18790 is a Vite **dev** server (pid 1558, cwd `tinker-ui`) that transpiles on request.
- **Verified:** with a stubbed `openclaw` on PATH — default with a live ticket → **0** `chat.inject` calls; `TC_VOICE_BUBBLE=1` → exactly 1, with correct params; `agent:main:whatsapp:*` + opt-in → 0 (the pre-existing channel gate still holds). With the real unmuted config and only `paplay` stubbed, TTS still produced a wav and called `paplay` while making 0 injects — the audio path is intact.
- **Files:** `~/.local/bin/jarvis`, `~/.openclaw/workspace/skills/jarvis-voice/SKILL.md`, `~/.openclaw/workspace/skills/jarvis-voice/templates/VOICE.md`, and the new `verify:` gate in `tinker-ui.md`.
- **Rule:** **when a policy flips from "the tool does it" to "the agent does it", the tool must be made to STOP — a policy that only adds an author never removes one.** The 2026-08-31 change correctly told the agent to write the purple line and correctly told it not to wait for the script; it never disabled the script's half, so the system quietly acquired two authors for one line. And the second author was invisible to every instrument pointed at it: zero tokens hid it from billing, a path outside both repos hid it from grep, and a real, well-written fix for the same sentinel in a neighbouring function hid it from code review. **A duplicate that costs nothing is not cheap — it is unmeasurable.**

### FIXED [display-misclassify+event-ordering]: thinking-bubbles-promoted-to-answers-by-a-partial-narration-stamp (2026-09-02)

- **Symptom:** the owner, hours after the entry below was written and its fix shipped: _"You say the reasoning compaction was fixed and yet this same chat shows me a bunch of thinking messages as final answers. What is worse, it seems to have grouped all the tool calls in one single section, which is correctly compacted, which to me it hints towards not having respected the time of arrival of each message."_ **That second sentence is the whole diagnosis** — the class split was real, and it was visible from the outside without reading a line of code.
- **The measurement.** Run 68→278 of `agent:main:tinker:mtievarb`: 210 messages — **86 `toolcall+tool_result`, 63 `thinking`, 61 `text`** — and the SERVED history is perfectly interleaved (`h T C h T C …`), so nothing upstream reordered anything. Running the REAL `narrationIndices` over that run returns **60 narration + exactly 1 answer**, and the surviving answer is the bubble that opens `**Jarvis:**` and closes with the html summary. The rendered DOM showed the opposite: one `reasoning-group` holding **86 tool-rows** followed by **61 loose `msg assistant` bubbles**. Data right, classifier right, stamp wrong.
- **Root cause — a freeze that could be PARTIAL, and a read-back that treated "unclassified" as "answer".** Both gates asked `some(...)`: `if (!runMsgs.some(m => m._narration !== undefined) && settled)` to stamp, `if (runMsgs.some(m => m._narration !== undefined))` to read. So a stamp taken while the run held only its first few messages left every later member at `_narration === undefined` — and the read-back only adds `=== true` to `thinkingSet`. `undefined` is neither, so those messages were never classified, fell through to the answer branch, and stayed answers for the life of the tab. The chronology damage the owner spotted is a SECOND-ORDER effect, not a sorting bug: the renderer emits all intermediates and then all answers, so once 60 narration bubbles are misfiled as answers the run is displayed **by class instead of by time**.
- **My own contribution to it, on the record.** The entry below widened `runSettled` with `|| nothing-is-streaming` to unblock a stranded `_temporary`. That was correct for the bug it targeted but it made the stamp fire in MORE situations, including on runs that were still growing — so it raised the odds of exactly this partial stamp. The first fix was real and stays; it was half the story, and the half it left open is the half the owner hit next.
- **Fix:** the freeze is now all-or-nothing per run. `fullyStamped` requires `stampedCount === runMsgs.length`; anything short of a complete stamp is treated as stale and re-classified rather than half-read. The 2026-08-05 guarantee is untouched — a run that IS fully stamped still reads its stamp and is never re-shuffled — but "classified" and "unclassified" can no longer coexist inside one run.
- **Reproduced and verified deterministically on the real transcript** (stamp taken after only 6 of 210 messages had arrived): OLD gate → narration=1, **60 bubbles rendered as answers**; NEW gate → narration=60, **1 answer**. 60 wrongly-promoted + the 1 real answer is exactly the 61 loose bubbles counted in the DOM. `vite build` clean; `vitest run tinker-ui/src` → **47 files / 1,660 tests passed**.
- **The owner's offered heuristic, and why it was not needed.** He suggested keying on the reply's shape (`**Jarvis:**` opening, html summary closing). Useful as corroboration and it matched perfectly — the structural classifier independently picked that exact bubble as the single answer — but it was deliberately NOT implemented: a content marker is the `💬 ANSWER` failure mode this file already records twice, and it would bind the fork's renderer to one persona's wording. Structure decided it correctly; the marker only confirmed it.
- **Files:** `tinker-ui/src/app.ts`.
- **Rule:** a memoised verdict over a COLLECTION must be all-or-nothing. When the write is gated on "has anyone been stamped?" and the read treats a missing stamp as a default value, growth after the first stamp silently reclassifies every new member — and the default is invisible precisely because it looks like a decision.

### FIXED [display-misclassify+event-ordering]: reasoning-never-collapsed-because-a-\_temporary-flag-outlived-its-stream (2026-09-02)

- **Symptom:** the owner: _"the last response you sent is preceded by a lot of reasoning, that did not collapse correctly"_ — and the automatic reasoning compaction had been _"failing lately"_ in general.
- **The measurement.** On his own live tab (`agent:main:tinker:mtievarb`), the run ending in the 3,941-char answer renders as **~30 loose `msg assistant` bubbles with no `reasoning-group` at all**. Replaying the real classifier (`normalizeHistoryRenderBlocks` → `isRunBoundary` → `narrationIndices` → the grouping pass) over that run's SERVED history classifies **229 intermediates / 2 answers** — a clean collapse. The pure decision was never wrong; something downstream of it refused to use the answer. Across the tab, 9 `reasoning-group`s exist for the older runs and none for the recent ones, which is the shape of a defect that arms itself over a session's life rather than one that is always on.
- **Root cause — a streaming mark with no owner left in the session.** The render gate read `hasTemporaries = intermediateIndices.some(j => view[j]._temporary)` and folded it into `isStreaming`, which gates the collapsed branch. `_temporary` is cleared in exactly two places and BOTH match on the run id: `ownsTempMsg` promotes only a temp whose `_runId === p.runId`, and the frozen-reasoning loop clears only one whose `_reasoningRunId === p.runId`. A history reload then does `messages = incoming`, which **destroys every `_runId` stamp** — while `_isReasoning` IS in msg-order's `CLIENT_ONLY_FLAGS` and `_temporary` is **not**, so a live reasoning bubble is preserved across the reload verbatim, `_temporary` and all. The flag now names a run that can never recur, so nothing can ever clear it, and one stale temp pins its whole run to `isStreaming` **permanently**. The `_narration` freeze is gated on the same `!some(_temporary)` test, so the run could not even be classified and re-collapsed later — the two gates failed together, which is why no repaint, and no amount of waiting, ever fixed it.
- **Why it looked intermittent:** a tab that has never reloaded mid-life has no stranded temps and collapses correctly, which is most short sessions. The defect accumulates with reconnects — and the reconnect/retry machinery that performs those reloads without a manual refresh is itself recent (2026-08-23 / 08-28 / 08-30), which is exactly the window in which "lately" starts.
- **Fix (three edits, all in `tinker-ui/src/app.ts`):** (1) the preserve loop in `loadChat` strips `_temporary` from every bubble it carries across the reload and marks it `_reasoningFrozen`, the same freeze turn-end applies — safe because that loop is only reached after `liveWriter` has been ruled out, so a surviving temp is stale by construction; (2) the render gate asks the honest question — `hasTemporaries` now also requires `streamRunId !== null || streamMsgUid !== null`, i.e. that something is ACTUALLY streaming; (3) the `_narration` freeze accepts "nothing is streaming" as an alternative proof that the turn ended. During a live turn both stream globals are set, so the streaming path is bit-for-bit unchanged.
- **Not the blinking-chat-text bug:** that regression (removed twice, 2026-03-26 and 2026-05-29, bible §5.8) came from an `isCurrentRun` guard inside the narration CLASSIFIER, which made settled bubbles flip style on every delta. This guard sits only in the "has the turn ended" test and never touches classification.
- **Files:** `tinker-ui/src/app.ts`.
- **Verified:** `vite build` clean; `vitest run tinker-ui/src` → **47 files / 1,648 tests passed**. The UI is served by the Vite dev server from SOURCE (`:18790`), so the fix is live on the next tab reload — no gateway restart involved.
- **Found alongside, NOT fixed (separate defect, needs a call on what the user should see):** `normalizeHistoryRenderBlocks` rewrites any assistant message that carries `thinking` but no `text` into `rec.content = [{type:"text", …}]` — **replacing the whole array and destroying the `tool_use` blocks that lived in the same message**. Measured on this session: **86 tool blocks destroyed** in a 400-message window, **68 of them in the single failing run**. Compounding it, reloaded tool results arrive as role `toolResult` with blocks of type `text`, so `globalResultMap` indexes none of them and every reloaded tool row renders a permanent "running" spinner. Fixing it properly needs `renderMsg`'s early-returning `_isReasoning` branch to emit tool rows too, so it is a deliberate change to what reaches the screen rather than a one-liner.
- **SUPERSEDED IN PART, same day — read the entry above this one.** The diagnosis and fix here are correct but INCOMPLETE: the owner reported the same symptom hours later, from a different cause (a PARTIAL `_narration` stamp). The `runSettled` widening described above also made that second bug more likely by letting the stamp fire on runs that were still growing. Do not cite this entry alone as "reasoning compaction is fixed".
- **Rule:** a flag that means "this is happening right now" must be answerable from the thing that is happening, not from a mark left on the data. When the only clearers of a mark are keyed on an id that another code path is free to erase, the mark is not state — it is a leak with a timer.
...
| `mechanism-starved`                       | A complete, tested, deployed mechanism was handed an EMPTY input, so it ran faithfully and did nothing                                     |
...
| `answer-was-in-memory-but-not-in-context` | A fact the agent already had on record was never retrieved into the turn, because the question did not resemble the memory that answers it |
...
| `early-return-drops-attribute`            | A render path returns before the line that stamps a datum, so every feature keyed on it silently mounts nowhere and reads as unbuilt       |
| `fractal-anchor-is-live-only`             | State written only by a live socket handler and never persisted, so whatever depends on it dies on every page load and every reconnect     |
...
| `name-collision`                          | One name covers two unrelated surfaces, so a feature was built on the one the architect was not looking at and read as never shipped       |
| `observability`                           | A gauge under-reads the very quantity it exists to warn about, because its measurement scope is narrower than the volume billed            |
| `panels`                                  | A panel derives its numbers from a frozen literal, so it ages in one direction and ends up arguing for the more expensive choice           |
...
| `session-naming`                          | A session key lies about what the session IS, so grouping, retention, attribution and recovery are all corrupted at once, and silently     |
...
- **Fix (three commits):**
  ...
  3. **`9f7ec9f538`** — **suppress futile same-model re-resume**: when the aborted attempt produced zero content (`producedNoContent`), the idle-timeout failover no longer re-resumes the same model/session — re-resuming an unchanged fat transcript can only reproduce the stall.
...
- **Rule:** a webchat-triggered privileged action must run through a NARROW webchat-scoped RPC (privileged work happens server-side), never a direct `operator.admin` call from the UI. A one-shot's timeout must cover the real wall-clock (historically a COLD `claude` spawn ~15–20s; since 2026-09-01 this is an xAI Grok 4.6 HTTP call) — a tight timeout returns `null` and the feature silently no-ops. An entity with no lifecycle event of its own (a fresh clone) needs an explicit trigger, not a buffer fix. Never persist a transient in-flight/shimmer flag — a mid-flight save strands it true and a dedup guard then blocks the action forever. See session-naming.md §"Auto-title mechanism". **Model as of 2026-09-01: `xai/grok-4.6`** (was `claude-code/claude-sonnet-4-6`).
...
- **Symptom:** four wakes logged "delivered" across the day; the user saw none of them. Each previous fix (wake retry, chat.send user lane, fallback cron) made delivery MORE reliable — and every delivery kept vanishing.
- **Root cause:** the wake targeted `agent:main:main` (a real session running grok-4.5), while the user's chat is `agent:main:tinker:msf0vedp`. Evidence: sessions.list shows agent:main:main's last inbound message is the 15:27 longjob fallback reminder, handled silently by that session. The 12:19 "delivered" cron wake from the morning vanished the same way. "Delivered" means "accepted by the gateway" — it says nothing about which room the message entered.
...
### FIXED [display-misclassify+gate-blindspot+dist-gap]: subagent spawn dies with opaque "provider error" when the task string exceeds 128 KB — fixed in source, awaiting rebuild (2026-08-09; source fix 2026-08-22)
...
- **Fix shape:** pass large task payloads by file reference (write to a temp file, hand the child a path) or over the existing WS RPC body rather than argv; and map `E2BIG` to a specific, non-retryable error message instead of `provider_error`. Independently, make the spawn-error path consume budget (or trip a circuit breaker after N consecutive spawn failures) so a dead Overseer stops retrying and reports itself dead. Note that capping the transcript is **not** a free fix — the Overseer is specified to judge completion against everything, so truncation trades a hard crash for silent misjudgement; the by-reference fix preserves the semantics and should be preferred. If a cap is added anyway it must be a **byte budget**, not `windowTurns`.
- **Source fix (2026-08-22, this turn):** all three legs landed. (1) `overseerSpawnTask()` writes the briefing (task + full transcript) to `/tmp/tinkerclaw-overseer/*.md` and hands the child a short pointer — `spawnOverseer` no longer concatenates persona+transcript into `task`. (2) `classifyRawErrorMessage` maps `\be2big\b` to `spawn_e2big` with a named envelope ("The Overseer briefing was too large to start"), not generic provider_error. (3) `maybeRunOverseer` consumes one budget slot on any spawn failure; E2BIG deactivates immediately (same payload cannot succeed). 18/18 overseer tests pass including the new E2BIG + budget-consume cases. **NOT deployed:** a `tsdown-build` would compile 33 other dirty files and kill the live gateway (it runs from `dist/index.js`); same blast-radius miss as `[stale-plugin-bundle-destroys-cron-report-bodies]`. Next deliberate gateway restart that includes a rebuild picks this up. Recurred today as `⟦OVERSEER⟧ spawn E2BIG` on the cron-card session — the session had grown past 128 KB, exactly the predicted fuse. (Jarvis)
...
### FIXED [gate-blindspot+name-collision]: the fractal verdict dock never rendered — wrong argument arity, unchecked because tinker-ui has no typecheck (2026-08-11)
...
### FIXED [session-integrity+spec-drift]: the session store is a plain LRU, so a background lane evicts the user's real conversations (2026-08-11)
...
- **[recipe-resolvers-stopped-at-slug-dir]** (2026-09-02) **FIXED IN SOURCE, NOT YET DEPLOYED — the entry above was one sighting of a CLASS; four resolvers of "slug → file" each stopped at a different depth.** Census run while rewriting the coding recipes: (1) `scanRecipeDir` had been widened to two levels on 2026-08-22 (`cc41ba32f36`) but the library also nests a SUBDIVISION level (`writing/papers/*.md`, `operations/infra/*.md`) — **17 recipes** tab-listed and matcher-invisible (`list` 83 vs `match` 57); (2) the scanner scored only `tags:`, while every recipe/1.0 playbook declares its phrases as `triggers:` — so `feature`/`debug`/`refactor` matched on title+summary alone and every coding prompt came back `confidence:"low"` ("build me a feature that adds a login button" scored 4); (3) `recipe-runner.ts` `loadRecipeText` and (4) `prefrontal.recipe.read` probed `<own>/<slug>/recipe.md` only — the runner could not load a `uses:` target that lives in `coding/`, and the recipe page 404'd on the same slugs. Plus a fifth defect found by the probe that should have proven the fix: `loadRecipeIndex` keyed its cache on the ROOT dir's mtime, which does not move when a file inside a category folder is rewritten — the live gateway kept scoring the OLD `feature.md` until the root was `touch`ed (4/"low" → 20/"high" the same second). **Fix (column, not cell):** a new leaf module `extensions/tinkerclaw-prefrontal/recipe-locate.ts` is the ONE owner of the layout rule (`collectRecipeTargets`, `findRecipeFile`, `SCAN_MAX_DEPTH=3`; no imports from runner/matcher so it can never close a cycle); the scanner, the runner loader and the read RPC all resolve through it; `triggers:` are folded into the scored tags; the index cache is keyed on every recipe file's `(path, mtime, size)`. Tests: `__tests__/recipe-locate.test.ts` (new), plus cases in `recipe-matcher-smart`, `recipe-overlay`, `recipe-rpcs`. **The same run also found the matcher test file had been red for weeks**: 12 fixtures predate `antiTriggers` and `scoreRecipe` threw on them — hardened to `kit.antiTriggers ?? []` and the field made optional. **Not live:** the plugin runs from `dist/`; the recipe FILES are live (the scanner reads disk per call) — verified with `node scripts/broca.mjs match` after touching the library root. Rule: when the second sighting of a resolver bug arrives, list every resolver of the same key and fix them through one shared function; a per-site patch guarantees a third sighting. (Jarvis)
...
- **[model-label-collapse-hides-provider]** (2026-08-22) **OPEN — the same model on two providers renders as two identical rows, in three surfaces.** Reported as "gpt models duplicated or sometimes triplicated in the model selector and the models panel". They were never duplicates: `agents.defaults.models` held three routes to gpt-5.5 (`openai/`, `openai-codex/`, `github-copilot/`) and two to gpt-5.6-sol (`openai/`, `openai-codex/`) — different wallets, same model. Every surface then erases the distinguishing half of the key: (1) `tinker-ui/src/app.ts` `shortModelLabel()` maps any id matching `/sol\b/ && /gpt-?5/` to the bare string "Sol", so both Sol routes draw the same chip — deliberate (the provider logo was supposed to carry it), but the logo does not separate `openai` from `openai-codex`, which share OpenAI branding; (2) `renderAuthKeyRows()` labels with `modelName(id)`, same collapse; (3) the Config tab's Models card reads `m.id ?? m.name ?? m.model` and `m.provider` — **`models.list` returns neither `id` nor `provider`**, it returns `key` and `name`, so that card always falls through to `name` and prints the hardcoded literal "configured" in the provider column for every row. Surface 3 is a plain bug, not a design choice: the provider column has never once shown a provider. Repro: `openclaw models list --json | python3 -c "import json,sys,collections; print([k for k,v in collections.Counter(m['name'] for m in json.load(sys.stdin)['models']).items() if v>1])"` — any non-empty result is a set of rows the UI draws identically. Mitigated today by deduplicating the config (33 entries, zero colliding labels), which is a data fix and does not stop the next recurrence: adding any model that exists on two providers recreates it silently. Fix direction: make the label a function of the _whole_ key when a collision exists — compute the set of labels first, and only for colliding ones append a provider discriminator (`Sol · codex` / `Sol · api`); and in the Config card read `m.key` and derive the provider from its prefix instead of reading two fields the payload does not contain. LESSON: a label that drops part of a unique key is only safe while the dropped part is never the only difference — and nothing enforced that. (Jarvis)

- **[phantom-resume-one-restart-n-notices]** (2026-08-22) **OPEN — one real gateway restart produced three "the gateway restarted and interrupted your previous turn" injections, causing the same finished answer to be delivered three times.** Timeline, all verified: `systemctl --user show openclaw-gateway.service` reports `NRestarts=0` and `ExecMainStartTimestamp=Sat 2026-08-22 09:47:10 CEST`, and the journal shows one continuous `node[430875]` across the whole window — so there was exactly ONE restart, at 09:47. The resume prompt from `buildResumeMessage()` (`src/agents/main-session-restart-recovery.ts:221`) nevertheless arrived at **09:49, 09:52 and 09:53**, each time instructing a full orient-recover-continue cycle on work that was already complete and verified on disk. The journal confirms the answers were not lost: the full text of attempt 2 streamed at 09:54:52, and the bridge's own guard logged `[duprep] WARN BLOCKED duplicate assistant_cumulative emit`. Likely mechanism, from `src/gateway/server-startup-post-attach.ts:249`: boot calls `markRunningMainSessionsAsInterrupted({ sessionsDir })`, which marks EVERY session whose `status` is still `"running"`. `sessions.json` currently holds four such rows simultaneously — `agent:main:main`, `agent:main:tinker:mt0fgad1`, `agent:main:tinker:mt16qwe0`, `agent:main:fractal-reflection:1daa06ea` — and sessions appear never to leave `"running"`, so one boot fans out one resume per stale row and they all surface in the one visible tab. That makes the notice count a function of accumulated stale rows, not of restarts, and it grows over time. `isIdleCompletedTail()` (same file, ~L193) was written precisely to break phantom-resume loops and did not fire here — worth checking whether it is consulted on this path at all, since the tail in question was a completed assistant text message with no dangling `tool_use`. Repro: with 2+ sessions stuck at `status:"running"`, restart the gateway once and count the injected resume messages. Cost when it bites: the model re-verifies and re-delivers finished work N times, and each redelivery looks to the user like the agent is stuck in a loop — the opposite of the legibility the resume message exists to provide. LESSON: a recovery trigger keyed on a status field that nothing reliably clears degrades into a broadcast; the fan-out must be keyed on runs actually interrupted by THIS boot, and consumed once. (Jarvis)

- **[cron-card-title-was-the-job-name]** (2026-08-22) **SUPERSEDED same day.** First cut inverted the slots: put tonight's _report headline_ on the left and demoted the cron name to the right. The user: "the name of the cron on the left with more visible text, and its beginning of summary on the right, with less visible text (reversed as it is now) … not a summary of the issues found, but a summary of what the actual prompt of the cron is asking it to do." The identity/content mix-up was: ITEM titles are tonight's letters; CARD chrome is the cron's identity + its stable job. Findings never belong in the head. Replacement: `[cron-card-chrome-name-left-prompt-right]`.

- **[cron-card-chrome-name-left-prompt-right]** (2026-08-22) **FIXED (HMR).** Collapsed: registry `job.name` LEFT (loud, 13px/650), first-sentence skim of `job.description` (the prompt's ask, stable) RIGHT (quiet, muted). Expanded: name stays in the head; the lead is the full prompt description, not tonight's headline and not a second copy of the name. Schedule stays in the setup form (the right slot on an open card is hidden so it doesn't compete with the purpose paragraph). `cronCardHeadline` deleted; `cronCardChrome({name, description})` is the one paint helper. Census: one paint site (`cronCardHtml` in `app.ts`). 10/10 panel tests. Vite HMR, no rebuild. Appearance in the live tab UNVERIFIED this turn (no shared browser tab). (Jarvis)

- **[stale-plugin-bundle-destroys-cron-report-bodies]** (2026-08-22) **FIXED + PREVENTION WIRED.** Reported as a content problem: _"the titles are not prose ... the content is in most cases inadequate or inexistent"_ in the Crons lateral panel. It was not a content problem. The loaded `tinkerclaw-cron-panel` bundle (`dist/extensions/tinkerclaw-cron-panel/index.js`) was built **2026-08-19 10:50**, ~10h BEFORE `7b33b2e800c` landed the letter-shaped item parser that same evening (`cron-data.ts` 20:49, `board-store.ts` 21:54). So the running code had (a) no indented-continuation join in `parseReport` — verified by `grep -c 'deltas\[last\]' -> 0` in the live bundle against `1` in source, so **every bullet BODY was discarded at ingest**; (b) a `normalizeText` that flattened newlines; and (c) a `KIND_BY_TOKEN` holding only the six LEGACY tokens (`flag/changed/realized/dead/failed/note`), so every bullet using the six NEW `CRON-ITEM-VOICE.md` tags fell through to `{kind:"note", text:<whole bullet incl. the prefix>}` — which is why titles literally read `FOUND: ...` and every item skimmed as FYI regardless of whether it was an ASK or a BROKE. The cron fleet had meanwhile adopted the voice spec correctly on 08-20/21 (indented body lines per date: 08-19 **0**, 08-20 7, 08-21 66, 08-22 50), so three nights of good writing were destroyed on arrival. **Nothing alarmed because the panel still rendered a plausible one-line card.** Detector that settles it in one command, and the one I should have run first: a shape census on the stored data — `0 of 221` open items carried a title/body split. A uniform zero is a build artifact's signature; a writing-quality problem would show a spread. Fix: rebuild (`node scripts/tsdown-build.mjs`) + gateway restart, then `scripts/repair-cron-board-bodies.mts` refilled the bodies IN PLACE from the immutable report files using the plugin's own `parseReport`/`parseBullet` (exact title-line match on either key the old bundle could produce, so it can only restore a body our parser dropped, never invent one; never creates/deletes/reorders/retires an item, never touches `status`/`pinned`/`runs`/dismissal reasons). Restored **123 bodies across 10 boards**; 116 of 221 open items now render prose title + body + correct skim tag, verified through the live `cronpanel.board.get` RPC. The remaining ~105 predated the voice adoption, had no body anywhere on disk, and auto-resolve needs 3 missed runs — they only had 2, so they sat on the card as title-only junk. Follow-up same day (`scripts/retire-prevoice-cron-loglines.mts`): resolved those 100 in place (pinned/dismissed untouched) and re-tagged 9 leftover `FOUND:`/`BROKE:` prefixes the old map had filed as `note`. Census after: **121 open, 116 with a title/body split, 5 genuine one-liners** (2026-08-20 wind-down bullets that were written without a body). Spiritual-tech is now clear. **Prevention:** `scripts/check-runtime-staleness.mjs` compares every built artifact's mtime against its newest source file; negative-controlled against this incident, it reports an **11.1h** gap on the cron-panel target, i.e. it would have fired. Wired into the wind-down cron brief so it RUNS nightly and reports STALE as a BROKE bullet — a checker nobody runs is the decoy failure recorded in `[fractal-prompt-orphaned-decoy-files]` above. **Same root cause is named as a suspected cause in `[tinker-api-arbitrary-file-read]` and `[fractal-action-claim-detector-missed-recurrence]` (both this week): one stale Aug-19 bundle was silently gating at least four separate features.** LESSON: when output quality does not improve after the PRODUCER was fixed, verify the artifact that READS it was rebuilt before blaming the producer — a stale consumer destroys good input and presents as an unimproved producer. Second, honest miss: I ran the build and only afterwards checked `git status`, which had compiled **12 uncommitted source files** from peer sessions into the live runtime; all were fixes awaiting deploy so the outcome was benign, but the blast radius has to be known while it is still a choice. (Jarvis)

## OPEN [skills/outlook-hack] 2026-08-22 — reply-draft splice buries new text inside the quoted thread

`scripts/outlook-mail-fetch.mjs --patch-draft --keep-signature` splices before the FIRST `<div id="Signature">`. On a draft produced by Graph `POST /me/messages/{id}/createReply`, that first match is the QUOTED message signature, not the new reply, so the new body is inserted below the quote header (`<div id="divRplyFwdMsg">`) and the recipient opens the old thread first. Repro 2026-08-22 on "RE: Consulta LOPD": my text landed at offset 1987, quote block at 268 — PATCH returned 200 and the tool reported success. Fix: when `divRplyFwdMsg` is present, splice before IT (or before the leading `<hr>`), and only fall back to the signature div otherwise. Second gap: the SKILL.md advertises "Create email drafts" but the script has no create/reply function — `createReply` had to be hand-rolled against Graph. Add a `--create-reply <msgId>` verb (still send-safe; createReply only ever produces a draft).

### FIXED [mechanism-starved+config-dead-code]: `agents.defaults.model.fallbacks` was `[]`, so every rate limit ended a turn that four other supplies could have served (2026-09-04)

- **Symptom, in the architect's words (2026-09-03):** _"we are experiencing too many rate limitations (among other causes) that stall our thinking."_ A 429 on the primary produced a dead turn, not a slower one.
- **Root cause:** `src/agents/model-fallback.ts` holds a complete failover machinery — `runWithModelFallback`, `resolveFallbackCandidates`, auth-profile rotation, `FallbackSummaryError`, with tests — and `~/.openclaw/openclaw.json` sets `agents.defaults.model.fallbacks` to **`[]`**. `resolveFallbackCandidates` resolved that empty list faithfully every turn, so the ladder had exactly one rung (the primary) and there was nothing to fall back TO. Nothing was broken and nothing logged: the config applied exactly as written, the code ran exactly as designed, and the feature was inert.
- **Why it survived review.** This is not `config-dead-code` — that tag is for a key that _looked_ live and did not apply. Here the key applied perfectly; its VALUE was empty. Code review sees a mechanism, tests see a mechanism, and only the runtime config says the mechanism is fed nothing. Hence the new tag: **`mechanism-starved`**.
- **Fix:** THALAMUS now emits a PLAN rather than a model (`src/shared/thalamus-plan.ts`), whose `chain` is a recovery ladder of at most one rung per OTHER supply, and that chain is threaded to `fallbacksOverride` (`model-selection.ts` → `run.thalamusChain` in `queue/types.ts`, set in `get-reply-run.ts` → `resolveModelFallbackOptions` → `resolveEffectiveModelFallbacks`). Precedence is owned by ONE function: a user pin keeps its empty ladder; an agent-level `fallbacks: []` is an explicit decision not to fall back; an empty chain is byte-for-byte today's behaviour. Against the live 2026-09-04 board the ladder went from nothing to `grok-4.6 → gpt-5.6-sol → gemini-3.8-flash`.
- **Prevention, and it is the part worth copying.** The THALAMUS panel renders an EMPTY chain as an explicit **warning line**, not as blank space — the whole class is invisible precisely because "nothing there" and "nothing wrong" look identical. `routing-rationale-thalamus-v2.test.ts` pins the warning, and bible §5.86 records the decision and `auth-routing.md`'s verify block asserts the wiring survives. Absent renders as silence; empty renders as the alarm.
- **LESSON (generalise it):** when a feature is "implemented, tested and deployed" but has never demonstrably fired, check its INPUT before its code. The J-series registry (2026-08-02) had already recorded five instances of this exact shape — `when:`/`onError:`/`invoke skill:` directives present in the runtime and declared in ZERO recipe files — and nobody connected them to a config array. Same class, sixth sighting. The cheap detector is a census of the input, not a reading of the code: _how many production inputs actually reach this branch?_ A uniform zero is the signature. (Jarvis)
