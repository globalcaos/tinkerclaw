...
- **Segment LENGTH = token COST (Oscar 2026-06-13):** each turn's vertical length is proportional to a **weighted token count** so the segment's **AREA (width × length) ∝ that turn's total token cost**. Since `width` already = €/Mtok-OUTPUT, length ∝ `weighted = outputTokens + 0.2·inputTokens` (the 0.2 mirrors the ~5:1 output:input price ratio), giving `area ∝ price_out·(output + 0.2·input) = output_cost + input_cost`. Constants in `eeg-trace.ts`: `EEG_LEN_PER_WTOKEN` (px/token), `EEG_MIN_LEN` (≥ 2·ARC_HALF, keeps tiny turns clickable), `EEG_MAX_LEN` (caps a giant turn). Rows are token-sized and stacked cumulatively (newest at top); `ROW_H` now only sizes the EMPTY-paper placeholder. **Token feed:** output from the `stream:"effort"` final event; **input accumulated per runId from `round-start` `inputTokensEstimate`** (app.ts `eegInputByRun`). Approximate by design (the input/output split + 5:1 ratio are estimates) — the measured halo stays the truth channel.
