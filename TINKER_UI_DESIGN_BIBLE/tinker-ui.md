...
- **Status:** `DEPLOYED` (2026-06-11) — **relocated 2026-06-11** from the chat-area strip into the Models side panel.
...
- **Placement (relocated):** the slider lives in the **Models side panel** (`#budget-panel`, rendered by `updateBudgetPanel()` / `renderModelRow()` in `app.ts`), inserted **directly UNDER the ACTIVE tab's current model row**. Mechanics: `renderModelRow()` stamps a **`data-model-id`** anchor on each model row; `updateBudgetPanel()` locates the active session's model row by that anchor and injects `renderThinkingSlider()` via **`insertAdjacentHTML("afterend", …)`** so the slider sits immediately after that one row. The slider carries the class token **`model-think-slider`** (the `verify:` gate asserts `app.ts` contains this string).
...
- **Per-tab semantics / follows the active tab:** the slider's stop reflects the **active tab's session `thinkingLevel`**, and the Models panel **re-renders on tab switch via `refreshViewedSessionIndicators()`** (which triggers `updateBudgetPanel()`), so switching tabs moves the slider under the newly-active tab's model row and re-reads that session's level. Switching the Models panel's Session/All scope does not change which session the slider targets — it always targets the **active tab**.
- **DON'T-REGRESS invariants** (each is load-bearing — the slider was relocated _into_ machinery that punishes the obvious shortcut):
  ...
  3. **The slider is injected `afterend` of the active model row, not baked into the row markup.** `renderModelRow()` only adds the `data-model-id` anchor; `updateBudgetPanel()` owns the `insertAdjacentHTML("afterend", renderThinkingSlider())` placement so the slider survives independently of the per-row render and lands under exactly one (the active) row. Don't move the slider HTML into `renderModelRow()` itself (it would render under every model row).
...
- **Files:** `app.ts` (`renderModelRow()` `data-model-id` anchor; `updateBudgetPanel()` active-row lookup + `renderThinkingSlider()` injection via `insertAdjacentHTML("afterend", …)`; `model-think-slider` markup + change handler; the per-tab `sessions.update { thinkingLevel }` save; `refreshViewedSessionIndicators()` re-render on tab switch), `base.css` (slider styling)
