...
verify:
  ...
  - name: the jarvis voice script never authors the purple line a SECOND time (FORK 2026-09-06)
    cmd: python3 -c 'import os; p=os.path.expanduser("~/.local/bin/jarvis"); assert os.path.isfile(p), "voice script missing at ~/.local/bin/jarvis - it is untracked by design (it lives in neither repo, which is exactly why this bug took months to locate); if it moved, move this gate with it"; L=open(p).read().split(chr(10)); idx=[k for k,l in enumerate(L) if "chat.inject" in l and not l.strip().startswith("#")]; conds=[[l for l in L[:k] if l.strip().startswith("if ")][-1] for k in idx]; bad=[c for c in conds if "TC_VOICE_BUBBLE" not in c]; assert not bad, "the voice script injects a Jarvis bubble with no TC_VOICE_BUBBLE opt-in. The agent ALREADY writes that line as the first line of its reply (workspace/IDENTITY.md speak_on_home_session; jarvis-voice VOICE.md: purple is in the reply, audio is extra), so an ungated inject authors the identity line TWICE and every Tinker turn renders two assistant bubbles. The duplicate costs zero tokens, so no billing check can see it. See bug-log.md the-voice-script-authored-the-purple-line-a-second-time. Offending condition: " + str(bad)'
  ...
  - name: §5.8U THALAMUS v2 — supply axis, Opus anchor, reserved gate, and a chain that reaches fallbacksOverride
    cmd: python3 -c 'import os; r=os.path.expanduser("~/src/tinkerclaw/"); sup=open(r+"src/shared/thalamus-supply.ts").read(); fea=open(r+"src/shared/thalamus-feasibility.ts").read(); pl=open(r+"src/shared/thalamus-plan.ts").read(); fr=open(r+"src/shared/thalamus-frontier.ts").read(); sc=open(r+"src/agents/agent-scope.ts").read(); rr=open(r+"tinker-ui/src/panels/routing-rationale.ts").read(); assert "effectiveCost" in sup and "SHADOW_LAMBDA" in sup, "shadow price gone — fairness would need a second selector (bible 5.8U)"; assert "openrouter: \"metered\"" in sup, "openrouter no longer metered — it could win on the burn term (bible 5.8U)"; assert "fanOutSupply" in sup, "clock-aware fan-out gone (bible 5.8U)"; assert "CAPACITY_HEADROOM" in fea, "capacity veto gone — a job larger than the window would route anyway (bible 5.8U)"; assert "chainFloor" in fr and "chainFloor" in pl, "recovery floor gone — chains collapse to the primary supply (bible 5.8U)"; assert "THALAMUS_ANCHOR_KEY" in fr and "claude-opus-5" in fr, "dial no longer anchored on Opus — balanced stops meaning Opus (bible 5.8U)"; assert "THALAMUS_RESERVED_RE" in fr, "reserved gate gone — Fable reachable by drifting (bible 5.8U)"; assert "thalamusChain" in sc, "chain no longer reaches fallbacksOverride — a 429 ends the turn (bible 5.8U)"; assert "no fallback" in rr, "panel no longer warns on an empty ladder (bible 5.8U)"'
