...
- **Status:** `BUILT` (2026-06-13) — implemented same night via ORCA (4 units, contract-first; 17/17 vitest + vite build green on the merged tree); approved by Oscar via visual questionnaire (10/10 answers, `.superpowers/brainstorm/113276-1781306690/state/server.log`); supersedes the FALLBACK CHAIN + CONFIGURED split of §5.13a and extends §5.8f/§5.8g.
...
- **Line encoding:**
  ...
  - **Thickness = COST of the (model × effort) combination** (Oscar's free-text upgrade, NOT static smartness): v1 = lookup table `relativeCost(model) × effortMultiplier(level)` → `width = clamp(1.5, 1.5·log2(1+cost), 7)px`, with model-rank buckets as fallback for unknown models. The table is a named export (`EEG_COST_TABLE`) so measured per-combination costs can replace estimates later — approximate is ACCEPTED; divergence is the point of the halo.
