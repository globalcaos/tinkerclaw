...
- **DON'T-REGRESS invariants:**
  ...
  6. The trunk BREAKS between EVERY call (each its own segment, `EEG_CALL_GAP` each side) AND more strongly at every prompt rule (`EEG_TURN_GAP` each side, finish + restart) — NO continuous spline; call gap < prompt gap so the hierarchy is visible (the user 2026-06-20). Prompt separators are CLEAR solid clickable rules; the per-call gaps are the finer rhythm. There are NO connector beziers between calls.
