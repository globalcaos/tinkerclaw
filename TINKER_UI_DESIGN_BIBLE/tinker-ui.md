...
- **Line encoding:**
  - **Color = provider brand** (full palette, Oscar's q6): anthropic `#E8702A`, openai `#10A37F`, google = rainbow `linearGradient` (#4285F4→#EA4335→#FBBC05→#34A853 cycling along the trace), deepseek `#4D6BFE`, mistral `#FA520F`, meta `#0668E1`, xai/local/unknown = neutral gray `#8A8F98`.
...
- **Segment LENGTH so AREA ∝ TOKENS used (Oscar 2026-06-13):** Oscar wants each turn's **AREA (width × length) ∝ the tokens it used**, so `length = (k · tokens) / width` (`eegSampleLength` divides by `eegCostWidthPx`). "tokens" = a weighted blend `weighted = outputTokens + 0.2·inputTokens` (output counts more, mirroring the typical ~5:1 output:input ratio Oscar asked to fold in). Because `width` is the cost-per-token identity, dividing by it makes the ink track **token VOLUME regardless of model** — more tokens = more ink, whatever the line's thickness. (NOTE: the first parallel build shipped area ∝ _cost_ — `length ∝ tokens`, no ÷width — `48e64328ea`; corrected to area ∝ _tokens_ per Oscar's literal words. The difference: cost double-encodes what `width` already shows; tokens add a new dimension.) Constants in `eeg-trace.ts`: `EEG_AREA_PER_WTOKEN` (px² ink / weighted token), `EEG_MIN_LEN` (≥ 2·ARC_HALF, keeps tiny turns clickable + fits the bezier), `EEG_MAX_LEN` (caps a giant turn). Rows are token-sized and stacked cumulatively (newest at top); `ROW_H` now only sizes the EMPTY-paper placeholder. **Token feed:** output from the `stream:"effort"` final event; **input accumulated per runId from `round-start` `inputTokensEstimate`** (app.ts `eegInputByRun`). Approximate by design — the measured halo stays the truth channel.
- **Persistence (q8) + PERMANENT retention (Oscar 2026-06-13):** the WHOLE session is retained (no drop-oldest — `EEG_MAX_SAMPLES` is a runaway guard, not a window) so all activity is visible by scrolling the paper. Trace state rebuilds on load by **backfill from the context-anatomy API** (`/tinker/api/context-anatomy/<sessionKey>?limit=500` — raised so reload restores the full session, not just recent turns; the §5.9 poll endpoint, mapping input/output tokens best-effort). Live feed = the `stream:"effort"` consumer (app.ts ~3270) + round/lifecycle events. The paper is scrollable within the session context; no placeholder strokes — segments appear only from real events (§5.9 no-placeholders precedent).
