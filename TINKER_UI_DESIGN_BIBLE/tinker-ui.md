...
- **Line encoding:**
  ...
  - **Thickness = COST of the (model × effort) combination** (Oscar's free-text upgrade, NOT static smartness): v1 = lookup table `relativeCost(model) × effortMultiplier(level)` → `width = clamp(1.5, 1.5·log2(1+cost·2), 7)px`, with model-rank buckets as fallback for unknown models. `EEG_COST_TABLE` is anchored to Oscar's Anthropic ratios (2026-06-13): **sonnet = 1 (base), opus = 5× sonnet, fable = 2× opus = 10×** (haiku ≈ 0.3); other providers pegged to the same sonnet=1 scale. The named export lets measured per-combination costs replace these estimates later — approximate is ACCEPTED; divergence is the halo's point. NOTE the mapping is **logarithmic**, so the displayed pixel ratio is COMPRESSED (opus is not literally 5× sonnet's pixels) — a 10× cost spread fits the 1.5–7px range, and the min-width floor flattens the cheap end. Monotonic-with-cost, not pixel-proportional.
...
- **Persistence (q8):** trace state rebuilds on load by **backfill from the context-anatomy API** (`/tinker/api/context-anatomy/<sessionKey>?limit=N`, the §5.9 poll endpoint) so a reload/restore shows the session's history, not an empty paper. Live feed = the `stream:"effort"` consumer (app.ts ~3270) + round/lifecycle events. The paper is scrollable within the session context; no placeholder strokes — segments appear only from real events (§5.9 no-placeholders precedent).
