...
verify:
  ...
  # This check is INVERTED — it asserts that the 2026-06-26 removal of the clone auto-title kick
...
4. **Auto-title** (LLM-generated topic phrase from the chat content) — fires on assistant-turn `end` at user-turn 1 then every `TAB_TITLE_INTERVAL` (=5) turns, via `generateTabTitle()` (`app.ts`). The user-meaningful customisation path; wins over the random fortune, sets `titleLocked`, AND persists server-side via `sessions.patch {cookiePhrase, cookiePhraseUserSet:true}`.
   ...
   - **u4-tab-naming (2026-06-25):** the MODEL PATH moved off the old direct Ollama (`localhost:11434`) call to the gateway RPC `sessions.suggestTitle`. The prompt-shaping rules above are unchanged. **u5 (2026-09-01):** that RPC now runs xAI Grok 4.6 (was cc-bridge Sonnet). See "Auto-title mechanism" below for the RPC, the 45s timeout, clone auto-naming, the doubled-title guard, and the in-flight shimmer. (Any earlier bible text saying "Gemini", "local Ollama", or "cc-bridge Sonnet" for the model path is obsolete.)
...
- (u4, 2026-06-25; model switched u5 2026-09-01) The auto-title model path is the **`sessions.suggestTitle` RPC** running **xAI Grok 4.6**. Never revert to a webchat `fork.subagents.spawn` (scope-rejected → no title), to cc-bridge Sonnet (burns Claude subscription tokens on a 4-word one-shot), or to a direct Ollama/Gemini/metered-API call. Keep `sessions.suggestTitle` in the READ_SCOPE group so the webchat can call it.
- (u4) Keep the title one-shot `timeoutMs` generous (**45s**) — a tight timeout makes the RPC return `null` and the tab silently never renames. The original 45s was sized for a cc-bridge `claude` cold-spawn (14–19s); Grok HTTP is typically 1–3s, but the cap stays so a slow call still titles the tab.
- (u4, REVERSED 2026-06-26) A freshly cloned tab KEEPS its parent's doubled-icon title — do NOT kick an auto-title on creation. The original `pendingTitleKickTabId` kick was removed on purpose because it "wiped the clone's identity before the user had even used it" (`app.ts:10083`); a clone renames later through the ordinary turn-end titler like every other tab. So `pendingTitleKickTabId` must not come back, and `cloneTab()` must not title the clone it just made — both are gated by `scripts/bible/session-naming-clone-title.mjs`. Don't "fix" clone naming by seeding the clone's message buffer either (dead end: the active clone reads the global buffer that `loadChat` overwrites).
...
## Auto-title mechanism — sessions.suggestTitle RPC + clone kick (DEPLOYED — u4 2026-06-25, model u5 2026-09-01)
...
**Model path: a dedicated gateway RPC, not a privileged subagent and not Ollama.** `generateTabTitle()` builds the prompt client-side, then the client helper `spawnTitleViaBridge` calls **`sessions.suggestTitle {sessionKey, prompt}`** (`app.ts`). The handler (`server-methods/sessions.ts`) runs **`suggestTitleViaBridge()`** (`server-methods/suggest-title.ts`): a one-shot `runEmbeddedPiAgent({provider:"xai", model:"grok-4.6", disableTools:true, modelRun:true, promptMode:"none", …})` in a throwaway `temp:title-suggest` session, returning the first non-empty payload text. **u5 (2026-09-01): xAI Grok 4.6.** Was cc-bridge `claude-sonnet-4-6` from u4 until then — a 4-word one-shot that burned Claude subscription tokens and cold-spawned a full `claude` CLI worker per rename. Grok is an HTTP completions call on the surplus-token lane. Still NOT the metered Anthropic API, NOT Ollama, NOT Gemini.

- **Why a new RPC (not the old path):** the title used to be generated by a webchat-issued `fork.subagents.spawn`, but that method is `operator.admin`-scoped and the Tinker UI is a webchat client (`operator.read`) → the spawn was scope-rejected and **no title was ever generated**. `sessions.suggestTitle` is added to the READ_SCOPE group (`method-scopes.ts`) so the webchat can call it; the privileged run happens server-side. (commit `ef337eb1dd`)
- **45s timeout (was 15s).** Originally sized for a cc-bridge `claude` cold-spawn (~14–19s). At `timeoutMs: 15_000` the run hit `FailoverError: LLM request timed out`, `suggestTitleViaBridge` returned `null`, and the tab silently failed to rename. Raised to **`timeoutMs: 45_000`** (commit `00c8e5b79b`). Grok HTTP is typically 1–3s; the cap stays so a slow call still titles the tab. The client `req()` has no timeout (it waits for the gateway), so 45s is the only cap. See `bug-log.md` FIXED [scope-mismatch+timeout-tuning] (2026-06-25).

**Clone auto-naming: shipped as an explicit kick, then REVERTED — a clone now keeps its parent title.** The ONLY automatic trigger is the assistant-turn `end` handler, and a **freshly cloned tab emits no turn-end event**, so it was never titled. u4 first closed that gap with a module-level `pendingTitleKickTabId = clone.id` set by `cloneTab()` and cashed in by `loadChat()` once the buffer refilled. **That kick was removed on purpose on 2026-06-26**: it fired before the user had said anything in the clone and so "wiped the clone's identity before the user had even used it" (`app.ts:10083`). The contract is now the opposite one — a clone **inherits its parent's doubled-icon title** and renames later through the ordinary turn-end titler, exactly like any other tab. (Seeding the clone's message buffer was also a dead end — an active clone reads the global `messages` buffer that `loadChat()` overwrites, and a forked clone already carries the parent's user prompts server-side.) The frontmatter check was still demanding the kick back until it was **INVERTED on 2026-08-03**; it now fails if `pendingTitleKickTabId` reappears in `app.ts`, or if `cloneTab()` titles the clone it just made. See `tinker-ui.md` §5.8i for the clone fork/transcript itself.
...
The doubled-title guard and shimmer-dedup live in `tinker-ui/src/app.ts` (HMR-live; commit pending — `app.ts` is a contended hot file); the clone kick that once sat beside them was removed on 2026-06-26 and must stay removed. The RPC, scope, and 45s timeout are committed (`ef337eb1dd`, `00c8e5b79b`).
