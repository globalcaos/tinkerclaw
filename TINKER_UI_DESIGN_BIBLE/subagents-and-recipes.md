...
verify:
  ...
  - name: every slug→file resolver goes through recipe-locate.ts, and triggers: are scored (FORK 2026-09-02)
    cmd: python3 -c 'import os; r=lambda p: open(os.path.expanduser("~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/"+p)).read(); m=r("recipe-matcher.ts"); assert "collectRecipeTargets" in m and "parsed.triggers" in m and "st.mtimeMs" in m, "recipe-matcher.ts no longer scans via recipe-locate / folds triggers: / keys the cache on file mtimes — coding playbooks go back to confidence:low and edits inside category folders go stale"; assert "findRecipeFile" in r("recipe-runner.ts") and "findRecipeFile" in r("recipe-rpcs.ts"), "runner loadRecipeText or prefrontal.recipe.read stopped resolving through recipe-locate.ts — a uses: target in coding/ becomes unloadable again"; assert "SCAN_MAX_DEPTH" in r("recipe-locate.ts")'
  - name: the coding pipeline composes its seven sub-recipes by slug (FORK 2026-09-02)
    cmd: python3 -c 'import os,re; d=os.path.expanduser("~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/recipes/"); f=open(d+"coding/feature.md").read(); uses=re.findall(r"^uses: ([a-z-]+)$", f, re.M); assert uses==["brainstorm-gate","implementation-plan","parallel-build","test-hardening","verification-gate","code-review","finish-branch"], uses; [open(p) for p in [d+"brainstorm-gate/recipe.md", d+"implementation-plan/recipe.md", d+"coding/parallel-build.md", d+"coding/test-hardening.md", d+"verification-gate/recipe.md", d+"coding/code-review.md", d+"coding/finish-branch.md"]]'
  ...
  - name: Overseer DERIVES its loop bound EVERYWHERE (no frozen cap) — the per-turn engine (src/fork/overseer.ts) AND the recipe-runner loop both use deriveOverseerLoopBudget (canonical in src/fork, re-exported to the extension); right-side keep-going bubble via onKeepGoing
    cmd: test -f ~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/recipes/overseer/recipe.md && grep -Fq 'loop: until OVERSEER_DONE' ~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/recipes/overseer/recipe.md && grep -Fq 'export function deriveOverseerLoopBudget' ~/src/tinkerclaw/src/fork/overseer-budget.ts && grep -Fq 'deriveOverseerLoopBudget' ~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/overseer-budget.ts && grep -Fq 'deriveOverseerLoopBudget(' ~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/recipe-runner.ts && grep -Fq 'onKeepGoing' ~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/recipe-runner.ts && grep -Fq 'deriveOverseerLoopBudget' ~/src/tinkerclaw/src/fork/overseer.ts && grep -Fq 'OVERSEER_LOOP_HARD_CEILING' ~/src/tinkerclaw/src/fork/overseer.ts && ! grep -Fq 'export const MAX_OVERSEER_ITERATIONS' ~/src/tinkerclaw/src/fork/overseer.ts
  - name: Overseer nudges are FULL-CONTEXT completion DIRECTIVES (not "keep going") + the loop auto-engages on multi-step / won't-finish-in-one-turn tasks (not only explicit keywords)
    cmd: grep -Fq 'CONCRETE COMPLETION DIRECTIVE' ~/src/tinkerclaw/src/fork/overseer.ts && grep -Fq 'FULL CONVERSATION SO FAR' ~/src/tinkerclaw/src/fork/overseer.ts && grep -Fq 'windowTurns != null ? messages.slice' ~/src/tinkerclaw/src/fork/overseer.ts && grep -Fq 'looksMultiStep' ~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/index.ts && grep -Fq 'multi-step plan' ~/src/tinkerclaw/extensions/tinkerclaw-prefrontal/index.ts
  ...
  - name: P0 recipe-vars.json — tracked in the PRIVATE jarvis-brain repo (recoverable on crash), NEVER the public fork; non-credential config only
    cmd: grep -Fq '!recipe-vars.json' ~/.openclaw/.gitignore && ! git -C ~/src/tinkerclaw ls-files | grep -q 'recipe-vars.json'
...
The in-extension `emitTrail` helper (`index.ts`, the `before_prompt_build` matching half — distinct from the CLI `--trail`) gained an optional 4th `payload` arg broadcast on the `prefrontal-trail-event` lifecycle event alongside the prose message. Payload fields: `recipeId`, `confidence` (`none`/`low`/`high`), `score`, `matches`, `catalogSize`, `composedFrom`, `semanticInvoked`, `recoveredBySemantic`, and (FORK 2026-08-28) `recipeTitle` + `recipePath` on `matched`/`merged` — taken off the matcher's index entry so the chat can draw a one-line 🍳 notice under the prompt that matched, linking the recipe's own `recipe.md`. The chat-chip RENDER is owned by `tinker-ui.md` §5.8N; this optic owns the producer fields. Added to the `matched` / `merged` / `searched` / `composed` emits so the UI can render confidence chips + composition sub-lines from `d.payload` instead of re-parsing prose. (Chip/sub-line styling owned by `tinker-ui.md`.)
...
- **`triggers:` ARE scored (FORK 2026-09-02).** The line that used to sit here — "the kit catalog's `triggers` are documentation only" — was the reason every recipe/1.0 coding playbook matched at `confidence:"low"`: the scanner read `tags:` and ignored `triggers:`. Both lists fold into the scored tags now. The matching stays lexical + fuzzy; do not over-engineer it, but do not regress the fold.
- **The library layout has ONE owner: `recipe-locate.ts`** (`collectRecipeTargets` / `findRecipeFile` / `SCAN_MAX_DEPTH=3`, a leaf module with no runner/matcher imports). The matcher's scanner, `recipe-runner.loadRecipeText` and `prefrontal.recipe.read` MUST all resolve a slug through it. Three layouts are legal — `<slug>/recipe.md`, `<category>/<name>.md`, `<category>/<subdivision>/<name>.md` — and on 2026-09-02 each resolver honoured a different subset, so the matcher could select a recipe the runner then could not load (`feature` → `uses: parallel-build` in `coding/`). A resolver that re-implements the probe inline is the bug returning. `loadRecipeIndex`'s cache is keyed on every recipe FILE's `(path, mtime, size)`, never on a directory mtime — an in-place rewrite inside `coding/` does not move the root's mtime, and the live gateway served stale scores for exactly that reason.
- **The coding pipeline is a composed recipe (2026-09-02).** `coding/feature.md` v2 is `uses:` × 7 — `brainstorm-gate → implementation-plan → parallel-build → test-hardening → verification-gate → code-review → finish-branch` — then a code-free report. Each sub-recipe also runs standalone and exchanges the shared artifacts `{{plans_dir}}/YYYY-MM-DD-<feature>-{spec,plan}.md`, the edit-unit shape `{id, task, writes, reads, complexity, wave}` that ORCA consumes, and a `Ruling: <what> — <why> — <cost if wrong>` ledger. Questions are front-loaded into `brainstorm-gate`; after its approval gate every implementation choice is a logged ruling, and only four things stop a run (irreversible op, security-sensitive action, side effect outside the worktree, a plan where every path is a guess). Renaming a slug in that chain breaks `feature`; the CATALOG "Coding" row lists the set.
...
- **ORCA Phase-A `DOC_REQUIREMENT` + `MECHANISM_REQUIREMENT` (2026-09-01).** Every draft prompt carries both. (1) New design principles, features, and requirements are annotated in the owning bible optic in the SAME change; old entries the change makes wrong are updated in the same run — a duty, not an "also consider". Callers composing a run include the bible unit up front when they already know the change is one of those. (2) Code buys consistency, prompt buys plasticity; prefer code whenever possible, keep prompt when plasticity is the point or there is no structural producer. A unit must choose and say which. Don't regress either constant out of `draftPrompt` — a declared-but-uninjected rule is a rule the Phase-A model never sees (design-principles #21/#22/#23).
...
First real U11 instance: `coreyhaines31/marketingskills` (MIT). Structural facts:

- **Vendor home:** `~/.openclaw/workspace/vendor/<repo>/` — a plain git clone, NEVER added to the workspace git index (nested repo), updated by `git pull`. Vendored files are read-only: patch via a wrapper skill or an upstream PR, never in place.
- **Activation = ONE router skill per vendor repo (AMENDED 2026-06-13, total-recall eviction; supersedes the day-one symlink-six).** The vendor's whole catalog is fronted by a single umbrella skill (`~/.openclaw/workspace/skills/marketing/SKILL.md`): ~130-token always-loaded description carrying the umbrella trigger phrases; on-demand body = read `.agents/product-marketing.md` first + a route table task→`vendor/.../skills/<x>/SKILL.md` (all 44). Standing cost is flat regardless of vendor catalog size — no growth cap. Don't regress to per-skill symlinks unless a specific framework proves it needs its own daily trigger; if one does, re-symlinking is one `ln -s` (loader follows symlinks — pp-\* precedent; both the add AND the eviction were verified live 2026-06-12/13 with NO gateway restart).
- **Recipes never needed this treatment — the matcher IS total recall.** `recipe-matcher.ts` runs code-side at `before_prompt_build` over the on-disk catalog and injects ONLY the matched recipe's plan (`<active_recipe>`) or a small gap/low-confidence note; the catalog itself never enters the prompt (`catalogSize` is just a number in the gap note). Recipe storage is uncapped by design.
- **`marketing` recipe category:** in `CANONICAL_CATEGORIES` (`recipe-author.ts`, gates the author RPC only — `recipe-parse.ts` passes any category string through). 8 recipes carry it (7 recategorized + `marketing-page-audit`). Catalog color: coral.
- **Shared context contract:** `~/.openclaw/workspace/.agents/product-marketing.md` (upstream's own convention, adopted verbatim so vendored skills find it unmodified; symlinked into jarvis-workspace). Every marketing skill/recipe reads it FIRST and asks only what it doesn't cover. It owns the hard rules (PII boundary, no fabricated metrics, draft-only outbound) so no imported framework can override them.
- **Authoring standards adopted upstream-style for OUR ClawHub skills:** per-skill `evals/evals.json` (prompt + assertions, LLM-judge runnable) and trigger descriptions written as user-phrase lists ("when the user says '…'"), not capability prose.
