...
- **Activation = symlink, curation is the token gate.** Only conversational-trigger-worthy skills get symlinked into `~/.openclaw/workspace/skills/<name>` (loader follows symlinks — pp-\* precedent; verified live 2026-06-12: symlinked skills appeared in-session WITHOUT a gateway restart). Everything else stays path-reachable for recipes (`vendor/.../skills/<x>/SKILL.md` as a step reading list) at zero per-session token cost. Current set: cro, copywriting, copy-editing, seo-audit, marketing-psychology, launch.
