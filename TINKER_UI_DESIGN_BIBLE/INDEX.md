...
| `design-principles.md`      | Codified design rules — numbered principles spanning code organization, concurrency, observability, process, and the gift-list discipline (#21–#23). Every other bible file's "don't regress" clauses point back here (2026-05-12; #22/#23 2026-09-01)                                                                               | hand                              |
...
- **No functionality without an entry** (`design-principles.md` #21, ratified 2026-08-17; widened 2026-09-01). New design principles, features, and requirements are not done when they run — they are done when the optic that owns the concern carries an entry stating the DESIGN CRITERIA (purpose, invariant, choice made, alternatives rejected and why), written in the same change rather than afterwards. The architect does not have to ask. If no optic owns the concern, open one and add its row to the Files table above. **Old entries the change makes wrong are updated in the same run** — a stale optic is worse than a missing one. **The entry outlives the code**: an implementation that is reverted, abandoned, or never worked keeps its entry — `status` flips, a dated line records what failed, the criteria stand. That way a failed build still leaves a lasting document behind.
- **The bible is the gift list** (`design-principles.md` #21/#23, ratified 2026-09-01). It records what the architect wants, whether the feature currently works or not. Status of the running code is observation. Dropping a want because the implementation died is the failure the gift list exists to prevent.
- **Code for consistency, prompt for plasticity** (`design-principles.md` #22, corrected 2026-09-01). Code does not beat prompt all the time. Prefer code whenever it is possible; keep prompt when the want is plasticity or there is no structural producer. Choose; do not collapse the distinction.
