...
**RECIPES panel is the SOLE owner of the per-run / per-subagent breakdown (FORK 2026-06-04).** The chat thinking-indicator went **binary**: it renders ONE consolidated row when the viewed session is busy (with a small `▸N` subagent-count badge), NOT one row per active run + per subagent. The full per-run / per-subagent decomposition — which run is in which phase, what each subagent is doing — now lives ONLY here in the right-panels RECIPES area (the prefrontal render ladder + decision trail + per-subagent vitals with their `↳ <task>` sub-lines), plus the collapsible subagent chat bubbles. This is a deliberate split of responsibility: the chat indicator answers "is the viewed session busy? (one bit + a count)"; the RECIPES panel answers "what exactly is each run/subagent doing?". A single session-level Stop is correct because it already calls the session-level `abort()`. The precedence contract that lets one binary chat bit and the detailed RECIPES decomposition stay derived from the SAME scoped run set without disagreeing is owned by `done-signals.md` (the §147 helpers feed both; `scopedActiveRuns()` is the one set both read — see also done-signals.md §2 and panels.md §147 above). The binary-indicator visuals (the consolidated row, the `▸N` badge) are owned by `tinker-ui.md`.
...
7. **One helper applies the FULL indicator set on EVERY viewed-session change — `refreshViewedSessionIndicators()` (FORK 2026-06-04).** Rule 6 named the requirement ("re-render on user-driven view changes") but each viewed-session-change site was still hand-rolling its own subset of the five updates and they drifted apart: `switchToTab` updated the prefrontal tree but forgot `updateBudgetPanel()`, so the MODELS glow kept indicating after a tab switch under `"session"` scope; `attachSessionToTab` forgot both `updateBudgetPanel()` AND `updatePrefrontalTree()`, so prefrontal froze on "thinking" until a scope toggle forced a re-render. The fix collapses the subset to ONE function — `refreshViewedSessionIndicators()` = `updateChat()` + `updateBtn()` + `updateSessionsPanel()` + `updateBudgetPanel()` + `updatePrefrontalTree()` — called from BOTH `switchToTab` and `attachSessionToTab` (and any future site that changes which session is viewed). This is the §147 single-source rule for view-change indicator refresh: a viewed-session-change site that updates only a subset is the drift bug returning. The five constituent updaters each still own their panel's content; this helper owns the _invariant that they all fire together on a view change_. (root-cause detail owned by `bug-log.md`; visuals — the MODELS glow, the prefrontal "thinking" — owned by `tinker-ui.md`.)

If you are adding a panel or a status readout that needs "busy/active/which runs": call a helper. If none fits, widen a helper and document the widening here — do not add a fifth derivation. That is the whole point of #18. Likewise, if you add a panel that must refresh when the viewed session changes, add its updater to `refreshViewedSessionIndicators()` — do not re-hand-roll the subset at each call site (rule 7).
