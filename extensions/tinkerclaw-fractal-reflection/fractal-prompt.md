...
If any surface is stale: name what became stale and where (file path or URL); add to `ripple-tracker.md` with `[ ]`; if the fix takes under 2 minutes, do it now; otherwise note it for the next planning session.
