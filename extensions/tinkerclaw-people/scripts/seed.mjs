...
// - exact lower
// - first-letter + last-name probe against email local-part (e.g. "Xavi Ortodó" -> "x.ortodo")
...
async function main() {
  ...
  for (const [primaryKey, bucket] of buckets.entries()) {
    ...
    // Prefer the hand-curated table name (e.g. "Xavi Ortodó") over the auto
    // contact "Xavier Ortodó SERRA" — it's shorter and matches how the user
