 ...
 * Original: src/memory/synapse/debate-architectures.ts
 */

import type { DebateParticipant, DebateCost, DebateConfig } from "./raac-protocol.js";
import {
  runDebate,
...
export async function moderatedTribunal(
  ...
  const framing = await moderator.propose(
    `Frame this debate question for a panel: ${task}`,
    moderator.role,
  );
  costs.push({
    phase: "frame",
...
export function recommendArchitecture(opts: {
  ...
  if (opts.stakesLevel === "low" || (opts.budgetUsd !== undefined && opts.budgetUsd < 0.2)) {
    return {
      architecture: "fan-out",
      reason: "Low stakes or tight budget",
