...
2026-07-07: pull the flight / hotel / car documents out of the user's **work
...
1. **Clickable Google Maps links** on every address the user has to _navigate to_,
...
2. **Completeness gate** — if any trip pillar is still TBD / unconfirmed /
   ...
   what to do, and stop. (the user, 2026-07-08: _"account for a trip where not all
   the details are finalized… enquire what to do instead of spamming me with a
   half-finished business plan."_)

## When to Use

- the user says he's travelling and wants his trip gathered / sent to WhatsApp.
...
missing. A pillar that the user _expects_ but whose doc is absent/`TBD`/unpriced =
...
From the user's ask, fix the destination, the travel window, and whether a car is
...
Only gather **the user's own** documents. Colleagues' tickets in the same email
...
**Google Maps links** — on the addresses the user _navigates to_, in this order:
...
merged with the user's actual tickets:
...
**Done when:** the user knows exactly what landed where.
...
- **Ask-don't-assemble on incomplete data** (Step 4) is the defining rule — never
  send a partial briefing without the user's go-ahead.
- the user's PII only; exclude colleagues' tickets from attachments (Rule 2).
- No irreversible outbound beyond the WhatsApp-to-self / Gmail-draft-to-self path
  without explicit ok (Rules 3/4). The message is to the user himself, so this is in
  bounds.
