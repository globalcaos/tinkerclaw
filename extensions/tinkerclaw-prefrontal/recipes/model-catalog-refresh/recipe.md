...
- A configured model AA no longer scores keeps its entry and drifts to the bottom. The user
...
The user is the **cost, prominently**, not a request for permission.
...
- **2026-08-15** — xAI was demoted as an "unsupported provider" for two weeks, so Grok 4.6
  (would rank 4) went undetected until the user spotted it by hand. Subscription-billed
