...
import { validateKitSpec } from "../kit-author.js";
...
describe("skillMdToKitSpec", () => {
  it("transpiles a SKILL.md with 3 numbered steps into a 3-step KitSpec", () => {
    const spec = skillMdToKitSpec(GOOD_SKILL);
  ...
  it("infers steps from `Step N -` headings too", () => {
    const spec = skillMdToKitSpec(STEP_DASH_SKILL);
...
describe("malformed SKILL.md is rejected before any write", () => {
  it("throws when frontmatter is absent", () => {
    expect(() => skillMdToKitSpec("# Just a heading\nno frontmatter here")).toThrow(/frontmatter/i);
  });

  it("throws when name is missing", () => {
    const md = `---\ndescription: "has a desc but no name"\n---\n\n### 1. Do\nbody`;
    expect(() => skillMdToKitSpec(md)).toThrow(/name/i);
  });

  it("throws when description is missing", () => {
    const md = `---\nname: no-desc\n---\n\n### 1. Do\nbody`;
    expect(() => skillMdToKitSpec(md)).toThrow(/description/i);
  });
  ...
  it("throws when no procedure / steps can be inferred", () => {
    ...
    expect(() => skillMdToKitSpec(md)).toThrow(/step/i);
  });

  it("rejects a name that cannot produce a safe slug", () => {
    const md = `---\nname: "../../etc/passwd"\ndescription: "evil"\n---\n\n### 1. Do\nbody`;
    expect(() => skillMdToKitSpec(md)).toThrow(/slug/i);
  });
});
