/**
 * FORK: Voice markers & E_phi computation (self-contained copy for extension).
 *
 * TypeScript port of CORTEX Appendix B.
 * E_phi = [Component A (8 linguistic features) || Component B (128-dim style embedding)]
 * Total: 136-dimensional persona fingerprint vector.
 *
 * Adapted from src/memory/cortex/voice-markers.ts — no external dependencies.
 */
...
const INFORMAL_MARKERS = new Set([
  "oh", "wow", "haha", "lol", "heh", "hmm", "ugh", "yep", "nope", "yeah",
  "okay", "ok", "sure", "hey", "hi", "yo", "well",
...
function l2Normalize(vec: Float64Array): Float64Array {
  ...
  norm = Math.sqrt(norm);
  if (norm === 0) {
    return new Float64Array(vec.length);
  }
...
export function computeFormality(words: string[]): number {
  ...
  for (const w of words) {
    if (FORMAL_MARKERS.has(w)) {
      formal++;
    }
    if (INFORMAL_MARKERS.has(w)) {
      informal++;
    }
  }
  for (const w of words) {
...
export function extractLinguisticFeatures(text: string): LinguisticFeatures {
  ...
  return {
    ...
    questionFrequency: sentences.filter((s) => s.trim().endsWith("?")).length / nSentences,
    technicalDensity: computeTechnicalDensity(words),
  };
}

/**
 * Pack linguistic features into a Float64Array (Component A of E_phi).
 */
...
// ---------------------------------------------------------------------------
// Component B: 128-dim style embedding (placeholder)
// ---------------------------------------------------------------------------
 ...
 */
export function computeStyleEmbedding(text: string, embedFn?: EmbedFn): Float64Array {
  if (!embedFn) {
...
/**
 * Compute E_phi -- the full 136-dimensional persona fingerprint.
 *
 * E_phi = [normalize(Component A) || normalize(Component B)]
 */
...
/** Cosine distance between two E_phi vectors. Range [0, 2]. */
export function ePhiDistance(a: Float64Array, b: Float64Array): number {
  if (a.length !== b.length || a.length !== E_PHI_DIMENSIONS) {
    throw new Error(
      `E_phi dimension mismatch: expected ${E_PHI_DIMENSIONS}, got ${a.length}, ${b.length}`,
    );
  }
  let dot = 0;
  let normA = 0;
...
/**
 * Compute E_phi baseline fingerprint from multiple text samples.
 * Returns the mean E_phi vector.
 */
export function computeBaselineEPhi(samples: string[], embedFn?: EmbedFn): Float64Array {
  if (samples.length === 0) {
    return new Float64Array(E_PHI_DIMENSIONS);
  }
