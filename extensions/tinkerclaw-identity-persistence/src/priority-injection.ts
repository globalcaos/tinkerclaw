/**
 * FORK: Priority-aware persona injection (self-contained copy for extension).
 *
 * Tier structure:
 ...
 *
 * Adapted from src/memory/cortex/priority-injection.ts.
 * Uses local estimateTokens instead of importing from engram/event-store.
 */

import { estimateTokens, type PersonaState } from "./persona-state.js";
...
export const CORTEX_PERSONA_CONFIG = {
  maxBudgetFraction: 0.05, // eta_max -- persona never exceeds 5% of context
...
export function renderTier1A(ps: PersonaState, opts?: RenderTier1AOptions): string {
  ...
  if (!opts?.hasSoulFile) {
    lines.push(`# Persona: ${ps.name} (v${ps.version})`, `## Identity\n${ps.identityStatement}`);
 ...
 * Currently disabled: redundant when SOUL.md defines the relationship.
 */
...
export function injectPersonaState(
  ...
  const fullContent = [
    renderTier1A(ps, opts),
    renderTier1B(ps),
    skipRelational ? "" : renderTier1C(ps),
  ]
    .filter(Boolean)
    .join("\n\n");
