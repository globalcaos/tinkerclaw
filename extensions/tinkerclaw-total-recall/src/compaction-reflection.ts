...
/**
 * Local copy of CompactionManifest from pointer-compaction-runtime.
 * Avoids an upstream import path escape.
 */
...
function countRetrievalStats(
  ...
  for (let i = 0; i < events.length; i++) {
    const e = events[i];
    if (e.kind !== "tool_call" || !isRecallToolCall(e.content)) {
      continue;
    }
    ...
    const result = events
      .slice(i + 1)
      .find((r) => r.kind === "tool_result" && r.turnId === e.turnId);
...
function classifySeverity(
  ...
  if (missRate > 0.6 || falsePositives > 3) {
    return "high";
  }
  if (missRate > 0.3 || falsePositives > 1) {
    return "medium";
  }
  return "low";
}
...
function buildDiagnosis(
  ...
  if (falsePositives > 0) {
    parts.push(`${falsePositives} false positive(s).`);
  }
...
function classifyCompactionSeverity(
  ...
  if (tokensEvicted > 40_000 || eventsCompacted > 100) {
    return "high";
  }
  if (tokensEvicted > 15_000 || eventsCompacted > 30) {
    return "medium";
  }
  return "low";
}
...
function buildCompactionSuggestions(
  ...
  if (severity === "high") {
    ...
    if (eventsCompacted > 100) {
      hints.push(
        `${eventsCompacted} events compacted — consider more frequent compaction triggers.`,
      );
    }
    return hints;
  }
...
export function appendReflectionJsonl(record: CompactionReflectionRecord, baseDir?: string): void {
