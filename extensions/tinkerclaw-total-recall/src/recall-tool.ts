...
import { combinedSearch, type SearchFilters } from "./search-index.js";
...
function packUnderBudget(
  events: ScoredRecallEvent[],
  maxTokens: number,
): { packed: ScoredRecallEvent[]; truncated: boolean } {
...
export async function recall(
  ...
  return {
    events: packed,
    totalTokens: packed.reduce(
      (sum, e) => sum + (e.event.tokens || estimateTokens(e.event.content)),
      0,
    ),
    queryCount: queries.length,
    truncated,
