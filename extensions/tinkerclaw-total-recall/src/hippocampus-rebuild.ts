...
/**
 * Minimal logger shim replacing the upstream createSubsystemLogger dependency.
 * Logs to console with a [hippocampus-rebuild] prefix.
 */
const log = {
  info: (msg: string) => console.log(`[hippocampus-rebuild] ${msg}`),
  warn: (msg: string) => console.warn(`[hippocampus-rebuild] ${msg}`),
  debug: (msg: string) => console.debug(`[hippocampus-rebuild] ${msg}`),
  error: (msg: string) => console.error(`[hippocampus-rebuild] ${msg}`),
};
