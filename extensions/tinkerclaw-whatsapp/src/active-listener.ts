/**
 * FORK: tinkerclaw-whatsapp active-listener registry.
 *
 * Copied from extensions/whatsapp/src/active-listener.ts (Task 8).
 * Tracks the currently-active whatsmeow listener per account so that
 * outbound send helpers can find the right socket.
 */
...
// WhatsApp shares a live socket between inbound and outbound runtime chunks.
// Keep this on a direct globalThis symbol lookup; the generic singleton helper
// was previously inlined during code-splitting and split the listener state
// back into per-chunk Maps.
