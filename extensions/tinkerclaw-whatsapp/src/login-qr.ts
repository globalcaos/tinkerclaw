/**
 * FORK: QR login flow using whatsmeow-node.
 * Copied from upstream login-qr-wm.ts with import paths updated
 * for the tinkerclaw-whatsapp plugin layout.
 */

import { randomUUID } from "node:crypto";
import { danger, info, success } from "openclaw/plugin-sdk/runtime-env";
import type { RuntimeEnv } from "openclaw/plugin-sdk/runtime-env";
import { renderQrPngBase64 } from "../../whatsapp/src/qr-image.js";
import { resolveWhatsAppAccount } from "./accounts.js";
import { createWmClient, disconnectWmClient, type WhatsmeowClient } from "./session.js";
...
export async function startWebLoginWithQr(
  ...
  try {
    const captureMod = await import("./history/live-capture.js");
    bindCapture = captureMod.bindWmHistoryCapture;
  } catch {
    // Will fall back to session.ts binding
  }
  ...
  if (bindCapture) {
    try {
      ...
      /* session.ts may have already bound it */
    }
  }
