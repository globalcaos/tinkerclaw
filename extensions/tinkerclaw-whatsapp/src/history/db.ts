/**
 * FORK: WhatsApp History Database
 * SQLite + FTS5 for full-text search across all messages.
 *
 * Provides the persistence layer for WhatsApp message history — used by
 * live-capture (real-time message events), import-export (WhatsApp .txt exports),
 * and migrate-json-store (one-time Baileys JSON migration).
 *
 * Wired in: barrel-exported from ./index.ts; consumed by live-capture.ts,
 * import-export.ts, migrate-json-store.ts, and the search/stats tool endpoints.
 */
