...
import { resolveMergedWhatsAppAccountConfig } from "../../whatsapp/src/account-config.js";
import { hasWebCredsSync } from "../../whatsapp/src/creds-files.js";
import type {
  DmPolicy,
  GroupPolicy,
  WhatsAppAccountConfig,
} from "../../whatsapp/src/runtime-api.js";
...
export function resolveWhatsAppAccount(params: {
  ...
  return {
    ...
    selfChatMode: merged.selfChatMode ?? rootCfg?.selfChatMode,
    dmPolicy: merged.dmPolicy ?? rootCfg?.dmPolicy,
    allowFrom: merged.allowFrom ?? rootCfg?.allowFrom,
    groupAllowFrom: merged.groupAllowFrom ?? rootCfg?.groupAllowFrom,
    groupPolicy: merged.groupPolicy ?? rootCfg?.groupPolicy,
    textChunkLimit: merged.textChunkLimit ?? rootCfg?.textChunkLimit,
    chunkMode: merged.chunkMode ?? rootCfg?.chunkMode,
