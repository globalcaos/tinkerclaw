 ...
 *
 * Part of tinkerclaw-whatsapp plugin. The adapter is consumed by the channel
 * registration code (Task 8) which passes it to upstream's message pipeline.
 */
...
class BaileysEventBridge extends EventEmitter {
  ...
  /** @internal */ wireEvents() {
    ...
    this.wmClient.on("connected", (_evt) => {
    ...
    this.wmClient.on("logged_out", ({ reason }) => {
      const err = new Error(reason) as Error & { output: { statusCode: number } };
