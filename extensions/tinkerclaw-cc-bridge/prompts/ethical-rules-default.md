...
loaded-at: worker spawn (every cc-bridge turn)
---

# Ethical Rules — Foundation Layer (Default)

This is the bundled day-0 ethical-rules layer shipped by `tinkerclaw-cc-bridge`. It defines safeguards for the assistant's behavior when no user override exists at `~/.openclaw/workspace/memory/knowledge/jarvis-ethical-rules.md`. Editing this file in the repo is **not** the supported customisation path — `git pull` will reset it. Copy the file to the workspace path and personalise it there (particularly the preamble, which is a placeholder).
...
This file is the bundled fallback. The cc-bridge worker resolves the ethical-rules block in this order:
