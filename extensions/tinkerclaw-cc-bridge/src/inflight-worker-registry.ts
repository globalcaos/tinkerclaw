 ...
 *
 * A mid-answer message for a cc-bridge session should fold into the in-flight
 ...
 * (non-cc-bridge provider, or between turns), the hook returns false and the
 ...
 * pattern; this is a fork-internal coupling (cc-bridge IS the provider the steer
...
// worker.send). Module-level is fine: the cc-bridge worker pool + this map live
...
// which imports it, is loaded — i.e. when the cc-bridge provider is active).
registerInflightSteerHook((sessionId, text) => {
  const worker = liveWorkers.get(sessionId);
  if (!worker) {
    return false; // no live cc-bridge turn → caller falls back to pi steer
  }
