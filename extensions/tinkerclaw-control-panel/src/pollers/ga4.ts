/**
 * FORK: tinkerclaw-control-panel — Google Analytics 4 poller (real traffic).
 *
...
async function accessToken(): Promise<string> {
  ...
  const sa = JSON.parse(fs.readFileSync(SA_PATH, "utf8")) as { client_email: string; private_key: string };
...
export const ga4Sessions: PollerFn = async (args) => {
  ...
  if (!/^\d+$/.test(property)) throw new Error(`ga4.sessions needs a numeric propertyId, got "${args}"`);
  const token = await accessToken();
  const res = await fetch(`https://analyticsdata.googleapis.com/v1beta/properties/${property}:runReport`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ dateRanges: [{ startDate: "today", endDate: "today" }], metrics: [{ name: "sessions" }] }),
  });
