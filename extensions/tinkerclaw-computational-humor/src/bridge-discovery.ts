...
import { vectorMean, vectorSub, vectorAdd, normalize } from "./vector-math.js";
...
export type LlmBridgeGenerator = (
  conceptA: string,
  conceptB: string,
  count: number,
) => Promise<Array<{ label: string; embedding: number[] }>>;
...
export async function discoverBridges(
  ...
  return qualified.toSorted((a, b) => b.quality - a.quality).slice(0, maxResults);
}
