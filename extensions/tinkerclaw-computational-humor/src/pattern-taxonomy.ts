...
function conceptPairs(ctx: ConversationContext): Array<{
  a: ConversationContext["recentConcepts"][0];
  b: ConversationContext["recentConcepts"][0];
}> {
...
registerPattern({
  ...
  fn(ctx, index) {
    ...
    return results.toSorted((x, y) => y.score - x.score).slice(0, 5);
  },
});
...
registerPattern({
  ...
  fn(ctx, index) {
    ...
    return results.toSorted((x, y) => y.score - x.score).slice(0, 3);
  },
});
