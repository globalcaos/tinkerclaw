...
describe("Cross-Extension Communication", () => {
  ...
  beforeEach(() => {
    cogDir = mkdtempSync(join(tmpdir(), "cognitive-"));
  });
  afterEach(() => {
    rmSync(cogDir, { recursive: true, force: true });
  });

  it("Total Recall writes shared state for other extensions", () => {
    writeFileSync(
      join(cogDir, "total-recall.json"),
      JSON.stringify({ active: true, baseDir: "/tmp/engram", version: "1.0.0" }),
    );
  ...
  it("Identity Persistence writes persona state for Computational Humor", () => {
    writeFileSync(
      join(cogDir, "identity-persistence.json"),
      JSON.stringify({
        active: true,
        persona: { name: "Jarvis", humor: { frequency: "medium", sensitivityThreshold: 0.7 } },
      }),
    );
  ...
  it("Learned Intuition writes personality nudge for Identity Persistence", () => {
    writeFileSync(
      join(cogDir, "personality-nudge.json"),
      JSON.stringify({ adjustments: [{ trait: "warmth", delta: 0.1 }] }),
    );
  ...
  it("Round Table writes traces to Total Recall if available", () => {
    writeFileSync(
      join(cogDir, "total-recall.json"),
      JSON.stringify({ active: true, baseDir: cogDir }),
    );
