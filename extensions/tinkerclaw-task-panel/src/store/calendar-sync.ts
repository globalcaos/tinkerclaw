...
/**
 * execFile, NOT exec — argv array, no shell. `from` and `to` arrive as RPC params, so under the
 * old `exec(\`gog … --from ${from}\`)` a caller passing a value containing backticks or $(…) got
 * arbitrary code execution. Same defect that ran /usr/bin/orca on this host on 2026-08-05 from
 * git-cache.ts; found by scripts/check-shell-interpolation.mjs on its first run.
 */
