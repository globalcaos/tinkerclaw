/**
 * FORK: tinkerclaw-task-panel — task CRUD + diff-aware import.
 *
...
export function addTask(cfg: ControlPanelResolvedConfig, input: TaskAddInput): TaskRow {
  ...
  if (!row) throw new Error(`[task-panel] addTask: failed to read back inserted row ${id}`);
  return row;
}
...
export function updateTask(cfg: ControlPanelResolvedConfig, input: TaskUpdateInput): TaskRow {
  ...
  if (!existing) throw new Error(`[task-panel] updateTask: no task with id ${input.id}`);
  ...
  if (!row) throw new Error(`[task-panel] updateTask: failed to read back ${input.id}`);
  return row;
}
...
export function dismissTask(
  ...
  if (!existing) throw new Error(`[task-panel] dismissTask: no task with id ${id}`);
  ...
  const row = getTaskById(cfg, id);
  if (!row) throw new Error(`[task-panel] dismissTask: failed to read back ${id}`);
  return row;
}
...
export function rescheduleTask(
  ...
  const db = getDb(cfg);
  const existing = getTaskById(cfg, id);
  if (!existing) throw new Error(`[task-panel] rescheduleTask: no task with id ${id}`);
  ...
  const row = getTaskById(cfg, id);
  if (!row) throw new Error(`[task-panel] rescheduleTask: failed to read back ${id}`);
  return row;
}
