/**
 * FORK: tinkerclaw-task-panel — task_est_preset CRUD (v3.3).
 *
...
export function addEstPreset(
  ...
  if (!Number.isFinite(input.minutes) || input.minutes <= 0) {
    throw new Error("[task-panel] addEstPreset: minutes must be > 0");
  }
  if (!input.label.trim()) throw new Error("[task-panel] addEstPreset: label required");
...
export function updateEstPreset(
  ...
  if (!existing) throw new Error(`[task-panel] updateEstPreset: no preset with id ${input.id}`);
  const fields: string[] = [];
  const params: Record<string, unknown> = { id: input.id, now: now() };
  if (input.minutes !== undefined) {
    if (!Number.isFinite(input.minutes) || input.minutes <= 0) {
      throw new Error("[task-panel] updateEstPreset: minutes must be > 0");
    }
