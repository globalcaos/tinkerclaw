/**
 * FORK: tinkerclaw-task-panel — task_axis CRUD (v3.3).
 *
...
export function addAxis(cfg: ControlPanelResolvedConfig, input: AxisAddInput): TaskAxisRow {
  const db = getDb(cfg);
  if (!input.id.trim()) throw new Error("[task-panel] addAxis: id required");
  if (!input.label.trim()) throw new Error("[task-panel] addAxis: label required");
  const existing = getAxisById(cfg, input.id);
  if (existing) throw new Error(`[task-panel] addAxis: axis ${input.id} already exists`);
  ...
  if (input.parent_id != null) {
    if (!validateParentDepth(cfg, input.parent_id)) {
      throw new Error(
        `[task-panel] addAxis: nesting beyond two levels is not supported (parent ${input.parent_id} is itself a sub-group, or does not exist)`,
      );
    }
  }
...
export function updateAxis(cfg: ControlPanelResolvedConfig, input: AxisUpdateInput): TaskAxisRow {
  const db = getDb(cfg);
  const existing = getAxisById(cfg, input.id);
  if (!existing) throw new Error(`[task-panel] updateAxis: no axis with id ${input.id}`);
  ...
  if (input.parent_id !== undefined && input.parent_id !== null) {
    if (input.parent_id === input.id) {
      throw new Error(`[task-panel] updateAxis: axis ${input.id} cannot be its own parent`);
    }
    if (!validateParentDepth(cfg, input.parent_id)) {
      throw new Error(
        `[task-panel] updateAxis: nesting beyond two levels is not supported (parent ${input.parent_id} is itself a sub-group, or does not exist)`,
      );
    }
  }
...
export function deleteAxis(
  ...
  if (attached > 0) {
    if (reassignTo === null) {
      throw new Error(
        `[task-panel] deleteAxis: ${id} has ${attached} tasks attached; pass reassign_to or set it to a string`,
      );
    }
    if (reassignTo !== id) {
      ...
      if (!target)
        throw new Error(`[task-panel] deleteAxis: reassign target ${reassignTo} does not exist`);
    }
  }
