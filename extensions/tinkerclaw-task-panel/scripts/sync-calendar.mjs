#!/usr/bin/env node
/**
 * FORK: tinkerclaw-task-panel — one-shot Google Calendar sync.
 *
 ...
 */
// execFileSync, NOT execSync — argv array, no shell. FROM/TO come from process.argv, so the old
// interpolated form executed anything a caller put in a date argument. See
// scripts/check-shell-interpolation.mjs for the incident this class caused on 2026-08-05.
...
try {
  const out = execFileSync(
    "gog",
    ["calendar", "events", "primary", "--from", FROM, "--to", TO, "--json"],
    {
      encoding: "utf8",
