...
**When knowledge graph is preferred:** When the relationship type between concepts matters for retrieval (e.g., "find all chunks that are _authored by_ the user" vs. "find all chunks that _mention_ the user"). The knowledge graph overlay is a natural HIPPOCAMPUS extension for structured domains.
