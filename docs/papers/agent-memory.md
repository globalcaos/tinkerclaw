...
```
Level 0: Raw facts     \$\\rightarrow\$ "the user mentioned he's working on the Ofertes project"
Level 1: Patterns      \$\\rightarrow\$ "the user typically works on commercial projects Mon-Thu"
Level 2: Principles    \$\\rightarrow\$ "the user values efficiency and dislikes unnecessary meetings"
Level 3: Strategic     \$\\rightarrow\$ "the user is building towards automating his workflow end-to-end"
```
...
```
Input:  "the user said he's been debugging the webhook handler all morning 
         and he's frustrated because the retry logic keeps failing silently"
Output: "the user debugging webhook handler; retry logic has silent failures"
```

**Track 2: Persona Compaction** (new)
```
Input:  "I told the user: 'That retry logic is sneaky — let's trace the failure 
...
Stance: peer-to-peer with the user. Style: concise, use code snippets, 
...
```
Level 3:  [the user is automating his workflow end-to-end]
              |
