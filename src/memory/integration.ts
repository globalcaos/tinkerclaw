...
import {
  ...
  loadProbeResults,
  type ProbeLLMFn,
} from "./cortex/behavioral-probes.js";
import {
  createDriftState,
  computeDriftScore,
  detectUserCorrections,
  type DriftState,
} from "./cortex/drift-detection.js";
import type { PersonaState } from "./cortex/persona-state.js";
import { createDefaultPersonaState, loadLatestPersonaState } from "./cortex/persona-state.js";
...
} from "./engram/pointer-compaction.js";
import { buildPushPack, type ContextPack } from "./engram/push-pack.js";
import { createDefaultTaskState, type TaskState } from "./engram/task-state.js";
import { computeSensitivityScore, type AudienceModel } from "./limbic/sensitivity-gate.js";
import type { ArchitectureResult } from "./synapse/debate-architectures.js";
import {
  createPersistentDeliberation,
  type PersistentDeliberation,
} from "./synapse/persistent-deliberation.js";
...
export class CognitiveOrchestrator {
  ...
  constructor(deps: CognitiveOrchestratorDeps) {
    ...
    try {
      loaded = loadLatestPersonaState(this.store);
    } catch {
      /* store may be broken */
    }
    ...
    if (this.config.synapse.enabled) {
      this.deliberation = createPersistentDeliberation({
    ...
    if (this.config.cortex.enabled) {
      ...
      try {
        ...
        if (driftScore.ewmaScore > 0.5) {
          ...
          injections.push(
            `[DRIFT ${severity.toUpperCase()}: score=${driftScore.ewmaScore.toFixed(2)}. Reinforce persona identity and voice.]`,
          );
          ...
          if (
            severity === "severe" &&
            this.config.synapse.enabled &&
            this.config.synapse.autoDebateOnSevereDrift &&
            this.runDebateFn
          ) {
    ...
    if (this.config.limbic.enabled) {
      ...
      try {
        ...
        if (sensitivity < humorThreshold && Math.random() < this.config.limbic.humorFrequency) {
          injections.push(
            "[LIMBIC: humor opportunity detected — consider light humor if appropriate]",
          );
          ...
          blocks: [
            {
              ...
              label: "system",
              tokens: estimateTokens(this.systemPrompt),
            },
            {
              role: "user",
              content: userMessage,
  ...
  getTurnMetricsHistory(): TurnMetrics[] {
    return [...this.turnMetricsHistory];
  }
  ...
  private recordTurnMetrics(tm: TurnMetrics): void {
    ...
    if (tm.engramMs > 0) {
      this.metrics.record("engram", "turn_ms", tm.engramMs, { turn: tm.turnNumber });
    }
    if (tm.cortexMs > 0) {
      this.metrics.record("cortex", "turn_ms", tm.cortexMs, { turn: tm.turnNumber });
    }
    if (tm.limbicMs > 0) {
      this.metrics.record("limbic", "turn_ms", tm.limbicMs, { turn: tm.turnNumber });
    }
    if (tm.synapseMs > 0) {
      this.metrics.record("synapse", "turn_ms", tm.synapseMs, { turn: tm.turnNumber });
    }
  }
}
