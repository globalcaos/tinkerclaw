...
describe("learning field", () => {
  it("learning field is non-empty for all severity levels", () => {
    ...
    const r1 = createCompactionReflector(store1).reflect(m1.id, {
    ...
    const r2 = createCompactionReflector(store2).reflect(m2.id, {
      contextTokens: 10_000,
      activeTopics: [],
    });
    ...
    for (let i = 1; i <= 5; i++) {
      addRecallPair(store3, i, EMPTY_RESULT);
    }
    const m3 = addCompactionMarker(store3, 6, SAMPLE_MANIFEST);
    const r3 = createCompactionReflector(store3).reflect(m3.id, {
      contextTokens: 10_000,
      activeTopics: [],
    });
  ...
  it("high-severity learning mentions hotTailTurns or anchor tags", () => {
    ...
    for (let i = 1; i <= 5; i++) {
      addRecallPair(store, i, EMPTY_RESULT);
    }
...
describe("JSONL persistence + daily digest", () => {
  ...
  it("getDailyDigest aggregates multiple records correctly", () => {
    ...
    const records: import("./compaction-reflection.js").CompactionReflectionRecord[] = [
      {
        timestamp: `${dateStr}T01:00:00.000Z`,
        severity: "low",
        diagnosis: "d1",
        suggestions: [],
        autoFixApplied: true,
      ...
      {
        timestamp: `${dateStr}T02:00:00.000Z`,
        severity: "medium",
        diagnosis: "d2",
        suggestions: [],
      ...
      {
        timestamp: `${dateStr}T03:00:00.000Z`,
        severity: "high",
        diagnosis: "d3",
        suggestions: [],
        autoFixApplied: false,
