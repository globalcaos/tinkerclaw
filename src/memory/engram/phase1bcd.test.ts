...
function populateStore(
  ...
  count: number,
  opts?: { kind?: MemoryEvent["kind"]; tokensPer?: number },
): MemoryEvent[] {
...
describe("Phase 1B: Pointer Compaction", () => {
  ...
  it("never evicts protected events (system_event, compaction_marker, persona_state)", () => {
    ...
    const systemEvent = store.append({
      turnId: 1,
      sessionKey: "compact-protect",
      kind: "system_event",
      content: "System init",
      tokens: 50,
      metadata: {},
    });
    const personaEvent = store.append({
      turnId: 2,
      sessionKey: "compact-protect",
      kind: "persona_state",
...
describe("Phase 1C: Task State", () => {
  ...
  it("preserves old versions in event store", () => {
    ...
    store.append({
      turnId: 0,
      sessionKey: "task-versions",
      kind: "system_event",
      content: JSON.stringify(ts),
      tokens: estimateTokens(JSON.stringify(ts)),
      metadata: { tags: ["task_state"] },
    });
...
describe("Phase 1D: Search Index (FTS)", () => {
  it("finds needle by keyword search", () => {
    ...
    for (let i = 0; i < 20; i++) {
      store.append({
        turnId: i,
        sessionKey: "search-needle",
        kind: "tool_result",
        content: `Generic output data ${i}`,
        tokens: 20,
        metadata: {},
      });
    }
    ...
    store.append({
      ...
      sessionKey: "search-needle",
      kind: "user_message",
      content: "The secret API key is xK9mP2qR7vL4",
      tokens: 15,
      metadata: {},
    });
...
describe("Phase 1D: Recall Tool", () => {
  it("recall finds needles after compaction", async () => {
    ...
    for (let i = 0; i < 30; i++) {
      store.append({
        turnId: i,
        sessionKey: "recall-needle",
        kind: "tool_result",
        content: `Build output line ${i}: ${"data ".repeat(50)}`,
        tokens: 100,
        metadata: {},
      });
    }
    store.append({
      ...
      sessionKey: "recall-needle",
      kind: "user_message",
      content: "The commit hash for the fix was a1b2c3d4e5f6",
      tokens: 15,
      metadata: {},
    });
  ...
  it("recall deduplication: excludes events in context", async () => {
    ...
    const ev = store.append({
      turnId: 1,
      sessionKey: "recall-dedup",
      kind: "user_message",
      content: "important secret data",
      tokens: 10,
      metadata: {},
    });
  ...
  it("recall budget enforcement", async () => {
    ...
    for (let i = 0; i < 50; i++) {
      store.append({
        turnId: i,
        sessionKey: "recall-budget",
        kind: "user_message",
        content: `Data point ${i}: ${"info ".repeat(100)}`,
        tokens: 200,
        metadata: {},
      });
    }
  ...
  it("recall batch queries return union", async () => {
    ...
    store.append({
      turnId: 1,
      sessionKey: "recall-batch",
      kind: "user_message",
      content: "The database host is db.example.com",
      tokens: 10,
      metadata: {},
    });
    store.append({
      ...
      sessionKey: "recall-batch",
      kind: "user_message",
      content: "The API endpoint is api.example.com/v2",
      tokens: 10,
      metadata: {},
    });
  ...
  it("needle survives compaction and is retrievable via recall", async () => {
    ...
    for (let i = 0; i < 40; i++) {
      store.append({
        ...
        sessionKey: "e2e",
        kind: "tool_result",
        content: `Build log line ${i}: ${"x".repeat(200)}`,
        tokens: 100,
        metadata: {},
      });
    }
    ...
    store.append({
      ...
      sessionKey: "e2e",
      kind: "user_message",
      content: "Set learning_rate=0.00042 and batch_size=128",
      tokens: 15,
      metadata: {},
    });
    // More noise
    for (let i = 50; i < 60; i++) {
      store.append({
        ...
        sessionKey: "e2e",
        kind: "tool_result",
        content: `More output ${i}: ${"y".repeat(200)}`,
        tokens: 100,
        metadata: {},
      });
    }
    ...
    const _cacheContents = cache.events.map((e) => e.content).join(" ");
