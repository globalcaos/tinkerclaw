...
import { vectorSearch } from "./search-index.js";
import {
...
describe("vectorSearch", () => {
  it("finds similar events by embedding", async () => {
    ...
    const e1 = store.append({
      ...
      sessionKey: "test",
      kind: "tool_result",
      content: "docker build failed with error",
      tokens: 10,
      metadata: {},
    });
...
describe("taskConditionedModifier", () => {
  it("task-relevant events score higher than cross-task events", () => {
    ...
    const sameTask = makeEvent({
      kind: "user_message",
      metadata: { taskId: "task-1", premiseRef: taskState.premiseVersion },
    });
    const diffTask = makeEvent({
      kind: "user_message",
      metadata: { taskId: "task-2", premiseRef: taskState.premiseVersion },
    });
  ...
  it("score is capped at M_MAX = 10.0", () => {
    ...
    const event = makeEvent({
      metadata: { tags: ["constraint"], premiseRef: taskState.premiseVersion },
    });
...
describe("recall with task-conditioned scoring", () => {
  it("ranks current-task events above cross-task events", async () => {
    ...
    store.append({
      ...
      tokens: 10,
      metadata: { taskId: "task-A", premiseRef: taskState.premiseVersion },
    });
    ...
    store.append({
      ...
      content: "nginx config error port 8080",
      tokens: 10,
      metadata: { taskId: "task-B", premiseRef: taskState.premiseVersion },
    });

    const result = await recall({ query: "nginx config error" }, store, undefined, undefined, {
      taskState,
    });
  ...
  it("push-pack contents change when task state changes", async () => {
    ...
    store.append({
      ...
      sessionKey: "test",
      kind: "tool_result",
      content: "build log output success",
      tokens: 10,
      metadata: {},
    });
    store.append({
      ...
      sessionKey: "test",
      kind: "user_message",
      content: "review the architecture decision",
      tokens: 10,
      metadata: {},
    });
    ...
    const debugResult = await recall(
      { query: "build output review" },
      store,
      undefined,
      undefined,
      { taskState: debugState },
    );
    const reviewResult = await recall(
      { query: "build output review" },
      store,
      undefined,
      undefined,
    ...
    if (debugResult.events.length > 0 && reviewResult.events.length > 0) {
      ...
      const debugToolScore =
        debugResult.events.find((e) => e.event.kind === "tool_result")?.score ?? 0;
      const reviewToolScore =
        reviewResult.events.find((e) => e.event.kind === "tool_result")?.score ?? 0;
      const debugUserScore =
        debugResult.events.find((e) => e.event.kind === "user_message")?.score ?? 0;
      const reviewUserScore =
        reviewResult.events.find((e) => e.event.kind === "user_message")?.score ?? 0;
