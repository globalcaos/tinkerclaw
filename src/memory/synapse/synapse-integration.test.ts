...
function makeMockParticipant(modelId: string, role: string): DebateParticipant {
  const profile =
    DEFAULT_PROVIDER_PROFILES.find((p) => p.role === role) ?? DEFAULT_PROVIDER_PROFILES[0];
  return {
    modelId,
    role,
    profile,
...
describe("RAAC protocol integration via SynapseRuntime", () => {
  it("runDebate produces finalSynthesis from mock participants", async () => {
    ...
    const result = await runDebate("Test topic", participants, {
...
describe("SynapseRuntime registry", () => {
