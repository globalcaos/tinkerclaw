...
export interface PersistentDeliberation {
  ...
  findResumableDebate(
    task: string,
  ): { debateId: string; lastRound: number; synthesis: string } | undefined;
}

export function createPersistentDeliberation(
  options: PersistentDeliberationOptions,
  ...
  function findResumableDebate(
    ...
    for (const e of conclusionEvents) {
      ...
      for (const tag of tags) {
        if (tag.startsWith("debate-")) {
          concludedIds.add(tag);
        }
      }
    }
    ...
    for (const e of traceEvents) {
      ...
      if (!debateTag || concludedIds.has(debateTag)) {
        continue;
      }
