...
describe("Phase 5A: Behavioral Probes", () => {
  ...
  describe("probe scheduling", () => {
    ...
    it("style fires every 5th turn", () => {
      const styleTurns = [];
      for (let t = 1; t <= 100; t++) {
        if (getScheduledProbes(t).includes("style")) {
          styleTurns.push(t);
        }
      }
    ...
    it("full_audit fires every 20th turn", () => {
      const auditTurns = [];
      for (let t = 1; t <= 100; t++) {
        if (getScheduledProbes(t).includes("full_audit")) {
          auditTurns.push(t);
        }
      }
    ...
    it("over 100 turns: hard=100, style=20, full=5", () => {
      ...
      for (let t = 1; t <= 100; t++) {
        ...
        if (probes.includes("hard_rule")) {
          hard++;
        }
        if (probes.includes("style")) {
          style++;
        }
        if (probes.includes("full_audit")) {
          full++;
        }
      }
...
describe("Phase 5B: Drift Detection", () => {
  ...
  describe("drift injection → detection latency", () => {
    it("detects drift within 3 turns of injection", () => {
      ...
      for (let t = 11; t <= 20; t++) {
        const probes = [
          makeProbeResult({ scores: { a: 0.2 }, violations: ["HR-001"], turnNumber: t }),
        ];
  ...
  describe("recovery test", () => {
    it("drift score recovers after corrections stop", () => {
      ...
      for (let t = 1; t <= 5; t++) {
        computeDriftScore(
          "too formal, be more casual",
          [makeProbeResult({ scores: { a: 0.3 } })],
          state,
          t,
        );
      }
...
describe("Phase 5C: Consistency Metric", () => {
  ...
  describe("consistency metric C", () => {
    ...
    it("action classification matches thresholds", () => {
      ...
      expect(classifyAction(0.8)).toBe("mild_reinforce");
      expect(classifyAction(0.6)).toBe("moderate_refresh");
      expect(classifyAction(0.4)).toBe("severe_rebase");
    });

    it("C formula: 0.6 * M_unit + 0.4 * (1 - Var)", () => {
      // With M_unit = 0.9 and Var ≈ 0 → C ≈ 0.6*0.9 + 0.4*1 = 0.94
      const probes = [
        makeProbeResult({ probeType: "hard_rule", scores: { a: 0.9 }, violations: [] }),
      ];
