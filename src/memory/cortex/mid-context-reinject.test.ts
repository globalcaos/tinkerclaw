...
describe("Phase 3.3: evaluateTurnSyncScore", () => {
  ...
  it("is a no-op when cortexRuntime is null", () => {
    // Should not throw
    expect(() => evaluateTurnSyncScore(null, ["Some assistant text."], 1)).not.toThrow();
  });
...
describe("Phase 3.4: Turn-based observational memory", () => {
  ...
  it("observation event JSON contains type, content, confidence", () => {
    ...
    extractor.extractObservations(["I believe clean code is more important than clever code."], 1);
  ...
  it("totalExtracted increments after each successful batch", () => {
    ...
    extractor.extractObservations(["I prefer functional programming patterns over OOP."], 1);
    const after1 = extractor.totalExtracted;

    extractor.extractObservations(["I always add type annotations to my TypeScript functions."], 1);
