...
function checkConvergence(state: ConvergenceState): void {
  ...
  if (thetaVariance > CONVERGENCE_CONFIG.varianceThreshold) {
    // Oscillation: alternating high/low θ — more disruptive than plain variance
    ...
    state.alerts.push({
      ...
      message: isOscillating
        ? `θ oscillating (var=${thetaVariance.toFixed(4)}, mean=${thetaMean.toFixed(3)})`
        : `θ variance ${thetaVariance.toFixed(4)} exceeds threshold ${CONVERGENCE_CONFIG.varianceThreshold}`,
  ...
  if (
    ...
    // Suppress duplicate alerts within the same divergence episode
    ...
    if (!lastAlert || lastAlert.turnNumber < tail[0].turnNumber) {
      state.alerts.push({
        ...
        timestamp: now,
        type: "diverging",
        message: `θ below ${CONVERGENCE_CONFIG.divergenceThreshold} for ${CONVERGENCE_CONFIG.divergenceConsecutive}+ turns (mean=${thetaMean.toFixed(3)})`,
        variance: thetaVariance,
        thetaMean,
      });
    }
