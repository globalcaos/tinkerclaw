...
describe("tts", () => {
  ...
  describe("sherpa-onnx provider", () => {
    describe("resolveTtsConfig", () => {
      it("resolves sherpaOnnx defaults when no config is provided", () => {
        const config = resolveTtsConfig({
          agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
          messages: { tts: {} },
        });
        expect(config.sherpaOnnx.bin).toContain("sherpa-onnx-offline-tts");
        expect(config.sherpaOnnx.modelDir).toContain("vits-piper-en_GB-alan-medium");
        expect(config.sherpaOnnx.lengthScale).toBe(0.5);
        expect(config.sherpaOnnx.effectsChain).toContain("flanger");
        expect(config.sherpaOnnx.effectsChain).toContain("aecho");
        expect(config.sherpaOnnx.effectsChain).toContain("treble");
        expect(config.sherpaOnnx.outputCodec).toBe("-c:a libopus -b:a 64k");
      });
      ...
      it("uses custom config when provided", () => {
        const config = resolveTtsConfig({
          ...
          messages: {
            tts: {
              sherpaOnnx: {
                bin: "/custom/path/sherpa-onnx-offline-tts",
                modelDir: "/custom/models/my-model",
                lengthScale: 0.8,
                effectsChain: "highpass=f=300",
                outputCodec: "-c:a libopus -b:a 128k",
                libPath: "/custom/lib",
              },
            },
          },
        });
        expect(config.sherpaOnnx.bin).toBe("/custom/path/sherpa-onnx-offline-tts");
        expect(config.sherpaOnnx.modelDir).toBe("/custom/models/my-model");
        expect(config.sherpaOnnx.lengthScale).toBe(0.8);
        expect(config.sherpaOnnx.effectsChain).toBe("highpass=f=300");
        expect(config.sherpaOnnx.outputCodec).toBe("-c:a libopus -b:a 128k");
        expect(config.sherpaOnnx.libPath).toBe("/custom/lib");
      });

      it("disables effects when effectsChain is empty string", () => {
        const config = resolveTtsConfig({
          agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
          messages: {
            tts: {
        ...
        expect(config.sherpaOnnx.effectsChain).toBe("");
      });
    });

    it("accepts sherpa-onnx as provider in TtsProvider type", () => {
      const config = resolveTtsConfig({
        agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
        messages: { tts: { provider: "sherpa-onnx" } },
      });
      expect(config.provider).toBe("sherpa-onnx");
    });

    it("accepts sherpa-onnx as provider in directive override", () => {
      const policy = resolveModelOverridePolicy({ enabled: true });
      const input = "Hello [[tts:provider=sherpa-onnx]] world";
      const result = parseTtsDirectives(input, policy);
      expect(result.overrides.provider).toBe("sherpa-onnx");
    });

    it("includes sherpa-onnx in provider order", () => {
      const order = tts.resolveTtsProviderOrder("sherpa-onnx");
      expect(order[0]).toBe("sherpa-onnx");
      expect(order).toContain("openai");
      expect(order).toContain("elevenlabs");
      expect(order).toContain("edge");
    });

    it("reports sherpa-onnx as configured when bin is set", () => {
      const config = resolveTtsConfig({
        agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
        messages: { tts: {} },
      });
      expect(tts.isTtsProviderConfigured(config, "sherpa-onnx")).toBe(true);
    });
  });
});
