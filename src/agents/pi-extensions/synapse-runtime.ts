...
const ROLE_CHALLENGE: Record<string, (proposal: string) => string> = {
  Architect: (p) =>
    `[Architect] Scalability concern: ${p.slice(0, 80)} lacks long-term adaptability.`,
  Critic: (p) => `[Critic] Logical gap: ${p.slice(0, 80)} fails under adversarial conditions.`,
  Pragmatist: (p) =>
    `[Pragmatist] Feasibility issue: ${p.slice(0, 80)} exceeds realistic constraints.`,
};

const ROLE_DEFEND: Record<string, (attacks: string[]) => string> = {
  Architect: (a) =>
    `[Architect] Rebuttal: addressed ${a.length} concern(s) by extending the design to cover edge cases.`,
  Critic: (a) =>
    `[Critic] Counter: the ${a.length} objection(s) confirm the need for stricter validation.`,
  Pragmatist: (a) =>
    `[Pragmatist] Mitigation: ${a.length} concern(s) resolved via staged rollout and fallback plan.`,
};
...
function computeDiversityScore(
  ...
  if (rounds.length === 0 || modelIds.length < 2) {
    return 0.5;
  }
...
export function createSynapseRuntime(eventStore: EventStore): SynapseRuntime {
  ...
  return {
    async debate(topic: string, options: DebateOptions = {}): Promise<DebateResult> {
      ...
      const confidence =
        votes.length > 0 ? votes.filter((v) => v === "accept").length / votes.length : 0.5;
      ...
      for (const round of result.rounds) {
        for (const [modelId, vote] of Object.entries(round.ratification)) {
          if (vote === "reject") {
            ...
            if (challenge) {
              dissent.push(`[${modelId}] ${challenge.slice(0, 120)}`);
            }
          }
        }
      }
      ...
      const actionItems = result.finalSynthesis
        ...
        .filter(
          (s) =>
            s.length > 10 &&
            /^(priorit|implement|deliver|rollout|validate|ensure|add|remove|refactor|test|monitor|review)/i.test(
              s,
            ),
        )
        .slice(0, 5);
