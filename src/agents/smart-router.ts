...
export class SmartRouter {
  ...
  public selectModel(taskDescription: string, contextTokens: number = 0): string {
    ...
    if (this.config) {
      const route =
        this.config.routes.find((r) => r.task_type === category) ||
    ...
    const validModels = chain.filter((modelId) => {
      ...
      const limit =
        this.config.strategies.context_waterline.models[modelId] ||
