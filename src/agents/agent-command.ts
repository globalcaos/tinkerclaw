    ...
    for (;;) {
      try {
        ...
        const fallbackResult = await runWithModelFallback<AgentAttemptResult>({
          ...
          // FORK: onFallbackStep dropped from runWithModelFallback signature in
          // chunk-23. Recorder is preserved on the call-site closure for future
          // re-wire when the new fallback-event surface is identified.
