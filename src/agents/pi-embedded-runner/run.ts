...
export async function runEmbeddedPiAgent(
  ...
  // FORK: When an explicit lane is provided (e.g. "cron"), use double-nesting for cross-lane
  // serialization. Otherwise, use only the session lane — each session runs independently
  // without a shared global lane that would serialize all interactive sessions.
  const hasExplicitLane = !!params.lane?.trim();
  const globalLane = hasExplicitLane ? resolveGlobalLane(params.lane) : null;
  const enqueueGlobal = globalLane
  ...
  // FORK: Only double-nest when an explicit global lane is provided (e.g. cron).
  // Interactive sessions use session lane only — no shared global lane.
  return enqueueGlobal ? enqueueSession(() => enqueueGlobal(runLlm)) : enqueueSession(runLlm);
}
