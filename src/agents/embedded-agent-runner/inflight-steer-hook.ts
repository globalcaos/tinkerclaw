 ...
 *
 * Lets a provider that owns a long-lived subprocess (today: tinker-bridge's
 ...
 *
 * Wiring: the tinker-bridge plugin registers a hook that writes the text to the live
 ...
 * no live provider worker for that session (non-tinker-bridge providers, or between
 ...
 * because the registrar (tinker-bridge) and the caller (core runs.ts) may live in
