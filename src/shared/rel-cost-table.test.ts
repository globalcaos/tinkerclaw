...
describe("shared/rel-cost-table", () => {
  describe("order preservation — first match wins", () => {
    ...
    // FORK 2026-09-04: GPT-6 Astra is metered cash at OpenAI's published $50 out. The
    // trap it must never fall into is the subscription block — the /fable|opus|sonnet|
    // haiku/ family rows and the seat-amortised gpt-5.6 rows are an ACCOUNTING
    // CONVENTION, not a price, so filing Astra beside them would draw 0.4465 and
    // understate the cash rate by 112x. Today exactly one row claims it; this pins
    // that, so the day someone adds a generic /gpt-6/ the sole-claimant assumption
    // fails loudly instead of silently re-pricing a frontier model.
    it("prices gpt-6-astra as metered cash, never on the subscription convention", () => {
      expect(allRowsMatching("openai/gpt-6-astra").map((r) => r.relCost)).toEqual([50.0]);
