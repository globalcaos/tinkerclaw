...
Burning your weekly Claude Max limit in two days?

It's not that you use too much AI — it's that you spend it in the wrong places. Heartbeats on Opus. Context that balloons every turn. Crons that never needed a genius.

Move the cheap work onto cheap models and you reclaim 80–90% of your budget for the work that actually matters. This is the exact playbook that keeps a 24/7 OpenClaw fork alive on a single account — Haiku heartbeats, session compaction, bash-ified crons — with the math behind each move.
...
**How to identify candidates:** If the job doesn't need to _understand_ or _generate_ text, it can be bash.
...
1. Open the file
2. Separate _identity_ (needed every message) from _reference_ (looked up occasionally)
...
_Compatible with OpenClaw 2026.2+_
