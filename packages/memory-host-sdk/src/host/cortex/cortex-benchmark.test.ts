...
import { computeConsistency, generateReinforcement, classifyAction } from "./consistency-metric.js";
import {
  createDriftState,
  computeDriftScore,
...
function buildPersona(): PersonaState {
  const ps = createDefaultPersonaState(
    "JarvisOne",
    "AI assistant and extension of the user. Technical, precise, moderately formal.",
  );
  ps.traits = [
    {
      dimension: "formality",
...
describe("CORTEX Benchmark 2: Drift recovery via persona re-injection", () => {
  it("re-injection corrects deliberate drift within 10 turns", () => {
    ...
    for (let t = 0; t < 20; t++) {
      const ds = computeDriftScore(
        `Tell me about ${TOPICS[t % TOPICS.length]}.`,
        [makeProbe(0.95, t)],
        driftState,
        t,
      );
      log.push({
        phase: "stable",
        turn: t,
        sync: +syncScore(ds.ewmaScore).toFixed(4),
        action: "none",
      });
    }
    ...
    for (let t = 20; t < 30; t++) {
      ...
      const consistency = computeConsistency(
        [makeProbe(0.2, t)],
        [OFF_PERSONA[(t - 20) % OFF_PERSONA.length]],
        baseline,
      );
      log.push({
        phase: "drift",
