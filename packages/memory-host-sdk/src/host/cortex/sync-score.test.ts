...
describe("SyncScore scheduling (every N turns)", () => {
  ...
  it("custom interval=5 evaluates on turns 5, 10, 15", () => {
    ...
    for (let t = 1; t <= 20; t++) {
      const logBefore = existsSync(join(tmpDir, ".openclaw", "engram", "sync-score-log.json"))
        ? readFileSync(join(tmpDir, ".openclaw", "engram", "sync-score-log.json"), "utf8")
            .split("\n")
      ...
      const logAfter = existsSync(join(tmpDir, ".openclaw", "engram", "sync-score-log.json"))
        ? readFileSync(join(tmpDir, ".openclaw", "engram", "sync-score-log.json"), "utf8")
            .split("\n")
            .filter(Boolean).length
        : 0;
...
describe("SyncScore JSONL logging", () => {
  ...
  it("log entries include needsReinjection flag matching threshold", () => {
    ...
    for (const line of lines) {
      const entry = JSON.parse(line) as SyncScoreResult;
      expect(entry.needsReinjection).toBe(entry.ewmaScore < SYNC_SCORE_DRIFT_THRESHOLD);
    }
  });
});
