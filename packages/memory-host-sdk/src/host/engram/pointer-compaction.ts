...
/** Approximate token cost of a single rendered time-range marker. */
const MARKER_TOKEN_ESTIMATE = 40;
...
export function estimateCacheTokens(cache: ContextCache): number {
  ...
  const markerTokens = cache.markers.length * MARKER_TOKEN_ESTIMATE;
...
function isEvictable(event: MemoryEvent, hotTailTurnIds: Set<number>): boolean {
  if (NON_EVICTABLE_KINDS.has(event.kind)) {
    return false;
  }
  if (hotTailTurnIds.has(event.turnId)) {
    return false;
  }
...
export function pointerCompact(
  ...
  while (estimateCacheTokens(cache) > targetTokens) {
    const allTurnIds = [...new Set(cache.events.map((e) => e.turnId))].toSorted((a, b) => b - a);
