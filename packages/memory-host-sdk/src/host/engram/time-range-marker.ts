...
export function renderMarker(marker: TimeRangeMarker): string {
  const topics =
    marker.topicHints.length > 0 ? ` Key topics: ${marker.topicHints.join(", ")}.` : "";
...
export function mergeMarkers(
  ...
  softCap: number = MARKER_SOFT_CAP,
): TimeRangeMarker[] {
  if (markers.length <= softCap) {
    return markers;
  }
 ...
 */
export function isMarkerWithinTokenCap(
  marker: TimeRangeMarker,
  cap: number = MARKER_TOKEN_CAP,
): boolean {
