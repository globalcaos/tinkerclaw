  ...
  return {
    ...
    getId(vector: number[]) {
      for (const e of entries) {
        if (cosineSimilarity(vector, e.vector) > 0.9999) {
          return e.id;
        }
      }
      return undefined;
...
describe("Phase 6B: Pattern Taxonomy", () => {
  ...
  it("callback pattern (10) requires previousHumor", () => {
    ...
    const ctxWithHistory: ConversationContext = {
      ...
      previousHumor: [
        { conceptA: "cat", conceptB: "dog", bridge: "pet", timestamp: new Date().toISOString() },
      ],
    };
...
describe("Phase 6C: Bridge Discovery", () => {
  ...
  it("LLM fallback activates when quality is low", async () => {
    ...
    const generator = vi
      .fn()
      .mockResolvedValue([{ label: "fallback", embedding: blendEmbeddings(A, B, 0.5) }]);
...
describe("Phase 6D: Humor Calibration", () => {
  it("shouldAttemptHumor respects frequency", () => {
    ...
    for (let turn = 0; turn < 1000; turn++) {
      if (shouldAttemptHumor(cal, turn)) {
        attempts++;
      }
    }
  ...
  it("shouldAttemptHumor modulated by audience receptivity", () => {
    ...
    let attempts = 0;
    for (let turn = 0; turn < 100; turn++) {
      if (shouldAttemptHumor(cal, turn, coldAudience)) {
        attempts++;
      }
    }
