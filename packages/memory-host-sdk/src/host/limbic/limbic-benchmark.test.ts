...
describe("LIMBIC Benchmark: Humor Quality & Bridge Validity", () => {
  it("scores 20 humor attempts and emits JSON report", async () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (best) {
        ...
        const approxSurprise =
          dist > 0 && scoredValidity > 0 ? scoredH / (dist * scoredValidity + 1e-9) : 0;
    ...
    const sensitivityResults = SENSITIVITY_CASES.map(({ a, b, bridge, expectBlocked }) => {
      ...
      return {
        pair: `${a}+${b}`,
        allowed: r.allowed,
        score: +r.sensitivityScore.toFixed(4),
        expectBlocked,
      };
    });
    ...
    const summary = {
      ...
      bridgeValidityRate: +(
        attempts.filter((a) => a.scored.validity > validityThreshold).length / 20
      ).toFixed(4),
      humorZoneHitRate: +(attempts.filter((a) => a.scored.inZone).length / 20).toFixed(4),
      sensitivityGateTriggerRate: +(
        sensitivityResults.filter((r) => !r.allowed).length / sensitivityResults.length
      ).toFixed(4),
    ...
    expect(summary.avgHumorPotential.scored).toBeGreaterThanOrEqual(
      summary.avgHumorPotential.random,
    );
  ...
  it("scored bridges have equal or better validity than random bridges", async () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const A = mockEmbedding(seedA),
        B = mockEmbedding(seedB);
      const index = buildIndex(i);

      const bridges = await discoverBridges(A, B, index, { minQuality: 0.0, maxResults: 3 });
      if (bridges[0] && bridgeValidity(bridges[0].vector, A, B) > threshold) {
        scoredValid++;
      }
      if (bridgeValidity(mockEmbedding(8000 + i), A, B) > threshold) {
        randomValid++;
      }
    }
