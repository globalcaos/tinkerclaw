...
function singleModelParticipant(_scenario: Scenario): DebateParticipant {
...
function buildErrorProfiles(rounds: DebateRound[]): Record<string, boolean[]> {
  ...
  for (const round of rounds) {
    for (const [modelId] of Object.entries(round.proposals)) {
      if (!profiles[modelId]) {
        profiles[modelId] = [];
      }
...
describe("SYNAPSE Benchmark: Debate Quality & Cognitive Diversity", () => {
  ...
  for (const scenario of SCENARIOS) {
    it(`[${scenario.id}] ${scenario.domain}: "${scenario.question.slice(0, 50)}..."`, async () => {
      ...
      const debateRounds = (debateResult as unknown as Record<string, unknown>).rounds as
        | DebateRound[]
        | undefined;
      ...
      const coverage = measurePerspectiveCoverage(
        debateResult.output,
        scenario.expectedPerspectives,
      );
  ...
  afterAll(() => {
    const report = {
      ...
      summary: {
        avgCDI: Number(
          (benchmarkResults.reduce((s, r) => s + r.cdi, 0) / benchmarkResults.length).toFixed(4),
        ),
        avgConsensusQuality: Number(
          (
            benchmarkResults.reduce((s, r) => s + r.consensusQuality, 0) / benchmarkResults.length
          ).toFixed(4),
        ),
        avgPerspectiveCoverage: Number(
          (
            benchmarkResults.reduce((s, r) => s + r.perspectiveCoverage, 0) /
            benchmarkResults.length
          ).toFixed(4),
        ),
        avgOutputEnrichmentRatio: Number(
          (
            benchmarkResults.reduce((s, r) => s + r.outputEnrichmentRatio, 0) /
            benchmarkResults.length
          ).toFixed(4),
        ),
