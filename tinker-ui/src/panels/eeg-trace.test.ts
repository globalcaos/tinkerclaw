...
describe("eegCostWidthPx", () => {
  ...
  // FORK 2026-08-12 (the user: "I would like to keep the linear axis") — reverted from
  ...
  // FORK 2026-08-13: the defect the user caught — Fable ($50/Mtok, the dearest model we
  ...
  // FORK 2026-08-12: the PROPERTY linear buys, and the reason the user kept it — the
  ...
  // FORK 2026-08-16 (the user: "double check that the EEG thickness of the models are
  ...
  // FORK 2026-08-15 (the user: "do not cap pixel width — the thickness comparison is the
...
// FORK 2026-08-28 (the user: "in the model selector, the big spenders are capped and the
...
//
// FORK 2026-08-28 #2 — the chip box is NO LONGER a constant. The user: "are the trace
...
describe("eegCostWidthLogPx — the bounded surfaces (the user 2026-08-28)", () => {
  ...
  // What log BUYS at the bottom, and the half of the user's report the top-end fix
...
describe("resolveEegPaint carries both scales (the user 2026-08-28)", () => {
...
// FORK 2026-09-02 (the user: "the cost of Fable 5.1 is not correct ... claude models cost
...
describe("branch never paints into the label row (the user 2026-06-23: weird max↔high loop on the labels)", () => {
  const TOP_PAD = 26;
...
describe("EEG concurrency = depth-shaded stack (bible §5.8h / §5.84, the user 2026-06-14)", () => {
  ...
  it("draws one strand per concurrent subagent with the BOTTOM whitened, front pure brand (the user 2026-07-20)", () => {
  ...
  it("a rainbow-provider stack stays the gradient but fades by depth (opacity, not tint)", () => {
    ...
    expect(ops[2]).toBeGreaterThan(ops[0]); // front solid, bottom of the pile faded (the user 2026-07-20)
  });
});

describe("tool calls branch off the trunk (the user 2026-06-25: scope C — see EVERY tool call, color+thickness over precise cost)", () => {
...
describe("close-stale + prompt anchors + prompt-break (the user 2026-06-19)", () => {
  ...
  it("draws the turn boundary as a YELLOW rule carrying data-eeg-prompt-text (the user 2026-06-22)", () => {
    const store = new EegTraceStore();
    store.record(sample({ runId: "r1" }));
    store.turnEnd({
      turn: 1,
  ...
  it("a subagent branch never draws above the label row (no max↔high loop on the labels)", () => {
    // the user 2026-06-23: a high subagent splitting off a max trunk near the TOP used to arc
...
describe("whitening only on REAL overlap + per-model lanes (the user 2026-06-25)", () => {
...
// FORK 2026-07-28 (the user "lines dancing laterally within Min"): a lane index must be
...
describe("lane offsets encode REAL concurrency (the user 2026-07-28)", () => {
...
describe("eegStrandShade — overlap pile whitens the BOTTOM (the user 2026-07-20)", () => {
...
// FORK 2026-08-06 (the user: central paint resolution). resolveEegPaint is THE
...
describe("resolveEegPaint — the central point (the user 2026-08-06)", () => {
...
// FORK 2026-08-06 #2 (the user: unify the thinking-indicator glows on the EEG
...
// FORK 2026-08-17 (the user: "nor the parallelism done, which should show a few EEG traces side
