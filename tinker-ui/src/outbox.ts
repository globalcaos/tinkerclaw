// FORK 2026-08-16 — bug (the user): "when you are changing the UI while I send prompts to Jarvis,
...
export type OutboxEntry = {
   ...
   * parked, not retired; only `reconcileWithHistory` may retire it. `clearAcks` re-arms every
   * entry on reconnect, because a new socket may mean a new gateway whose dedupe cache is empty —
   * and that is precisely when a genuinely unpersisted turn must be sent again for real.
   */
  ackedAt?: number;
};
...
/**
 * Re-arm every parked entry for replay. Called on RECONNECT only.
 *
 * A new socket may be a new gateway process, and the dedupe cache that produced the ack does not
 * survive a restart. An entry still unproven at that moment is one whose turn may never have been
 * persisted at all, so it must become eligible to be sent again for real. Returns how many were
 * re-armed, for the log line.
 */
export function clearAcks(store: OutboxStore): number {
  const entries = readOutbox(store);
  let n = 0;
  for (const e of entries) {
 ...
 *
 * FORK 2026-08-28 — bug (the user): every DEFERRED prompt was painted TWICE. The 5 s outbox backstop
 ...
 * `_undelivered: true`, and the user saw a solid amber "not delivered - will retry" bubble beside the
 ...
 *
 * REGRESSION FIX 2026-08-16 (the user: "I am still missing prompts"). The text fallback used to be
 ...
 * exactly the vanishing the outbox exists to prevent. The user demonstrably resends identical text
...
//
// FORK 2026-08-16, second pass (the user: "I am still missing prompts", after the outbox shipped).
//
...
// as it likes; the text the user typed is still on disk, timestamped, and readable. This is the store
...
export function dueForReplay(
  ...
  return entries.filter((e) => {
    ...
    // re-sending forever and destroying the entry. `clearAcks` re-arms these on reconnect.
    if (e.ackedAt !== undefined) {
      return false;
    }
