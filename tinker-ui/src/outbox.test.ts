...
describe("reconcileWithHistory", () => {
  ...
  // REGRESSION 2026-08-16 (the user: "I am still missing prompts"). Confirmation by text prefix used
  ...
  // then deleted from the transcript by the next loadChat. The user demonstrably resends identical
...
describe("an ack must never destroy the last copy", () => {
  ...
  it("re-arms parked entries on reconnect — a new gateway has no dedupe cache", () => {
    const store = fakeStore();
    enqueueOutbox(store, seed());
    markAcked(store, "id-1", NOW);
    expect(clearAcks(store)).toBe(1);
    expect(readOutbox(store)[0].ackedAt).toBeUndefined();
    // and it is replayable again, for real this time
    const due = dueForReplay(readOutbox(store), NOW + OUTBOX_REPLAY_GRACE_MS + 1);
    expect(due).toHaveLength(1);
  });

  it("clearAcks is a no-op when nothing is parked", () => {
    const store = fakeStore();
    enqueueOutbox(store, seed());
    expect(clearAcks(store)).toBe(0);
  });
