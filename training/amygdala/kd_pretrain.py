...
Strategy:
  1. For each historical situation (from mined_examples in training.sqlite):
     - Ask local LLM (Ollama llama3:8b) what personality modulation a
       heavily-prompted Oscar-persona agent would produce
...
def query_teacher_label(
    ...
    """
    Ask the LLM what personality vector a heavily-prompted Oscar agent would use.
...
adjustments a well-calibrated Oscar-style assistant would make. Oscar is technical, direct,
    ...
    try:
        ...
        if start >= 0 and end > start:
            ...
            label = [
                ...
                float(parsed.get("hedging", -0.3)),   # Oscar default: less hedging
                float(parsed.get("emoji_rate", -0.5)),  # Oscar default: low emoji
                float(parsed.get("assertiveness", 0.4)),  # Oscar default: assertive
            ]
            # Clamp to [-1, 1]
...
def heuristic_teacher_label(action_type: str, outcome_label: float) -> List[float]:
    ...
    Uses action type and outcome to approximate Oscar's personality adjustment.
    """
    # Base: Oscar's personality defaults
    base = [
