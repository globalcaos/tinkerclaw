...
const CHROME = process.env.UI_SHOT_CHROME ||
  "/home/globalcaos/.cache/ms-playwright/chromium-1134/chrome-linux/chrome";
...
try {
  const tok = process.env.UI_SHOT_TOKEN ||
    JSON.parse(fs.readFileSync(path.join(os.homedir(), ".openclaw", "openclaw.json"), "utf8"))?.gateway?.auth?.token;
  if (tok && !URL.includes("token=")) URL += (URL.includes("?") ? "&" : "?") + "token=" + encodeURIComponent(tok);
...
try {
  ...
  if (PULSE) {
    try { await page.click("#tb-exec", { timeout: 5000 }); } catch (e) { logs.push(`[warn] click #tb-exec: ${e.message}`); }
    await page.waitForTimeout(700);
    try { await page.click('.exec-tab[data-tab="pulse"]', { timeout: 5000 }); } catch (e) { logs.push(`[warn] click pulse tab: ${e.message}`); }
    // Metrics arrive over the gateway WS (several seconds) — wait for the
    // charts (or an error block) rather than a fixed delay.
    try { await page.waitForSelector(".pg-chart, .exec-kpi-error", { timeout: 25000 }); } catch { logs.push("[warn] no .pg-chart after 25s"); }
  ...
  try { await page.screenshot({ path: OUT, fullPage: true }); } catch { /* noop */ }
}
